...
#ifdef CONFIG_SCHED_HMP
	//int target_cpu; // modified by cyj on 20140925
#endif
...
#ifdef CONFIG_HMP_TRACER
				trace_sched_power_aware_active(
					POWER_AWARE_ACTIVE_MODULE_AVOID_BALANCE_FORM_CPUX_TO_CPUY,
					0, this_cpu, busiest->cpu);
#endif /* CONFIG_HMP_TRACER */
			...
			int prev_cpu, int new_cpu) 
{
	...
	memset(&B,0,sizeof(B));
	memset(&L,0,sizeof(L));
	/* do not overwrite the decision made by original cfs */
	if (cpumask_test_cpu(prev_cpu, &hmp_fast_cpu_mask) && cpumask_test_cpu(new_cpu, &hmp_fast_cpu_mask)) {
	    B_target = new_cpu;
	}
	if (cpumask_test_cpu(prev_cpu, &hmp_slow_cpu_mask) && cpumask_test_cpu(new_cpu, &hmp_slow_cpu_mask)) {
	    L_target = new_cpu;
	}
...
#endif
	step = 2;
	if ((hmp_cpu_is_slow(new_cpu)) && (hmp_up_migration(HMP_SELECT_RQ,se,L_target,B_target,&B,&L,&check)))
		goto select_fast;
	step = 3;
	if ((hmp_cpu_is_fast(new_cpu)) && (hmp_down_migration(HMP_SELECT_RQ,se,B_target,L_target,&B,&L,&check)))
		goto select_slow;
	step = 4;
	cpu = 0;
	if (hmp_cpu_is_slow(new_cpu)) {
		for_each_cpu(cpu, &hmp_fast_cpu_mask) {
