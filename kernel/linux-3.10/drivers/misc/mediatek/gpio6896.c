...
#define GPIO_INTR_CLR(gpio)     \
    __ckgen_writel(REG_GPIO_EXTINTEN + (GPIO_TO_INDEX(gpio) << 2) , GPIO_EXTINTEN_REG(gpio)  & (~(1U << (gpio & EDGE_INDEX_MASK))))
#define GPIO_INTR_FALL_SET(gpio)    \
    __ckgen_writel(REG_GPIO_ED2INTEN + (GPIO_TO_INDEX(gpio) << 2), GPIO_ED2INTEN_REG(gpio) & (~(1U << (gpio & EDGE_INDEX_MASK)))); \
    __ckgen_writel(REG_GPIO_LEVINTEN + (GPIO_TO_INDEX(gpio) << 2), GPIO_LEVINTEN_REG(gpio) & (~(1U << (gpio & EDGE_INDEX_MASK)))); \
    __ckgen_writel(REG_GPIO_ENTPOL + (GPIO_TO_INDEX(gpio) << 2), GPIO_ENTPOL_REG(gpio) & (~(1U << (gpio & EDGE_INDEX_MASK)))); \
    __ckgen_writel(REG_GPIO_EXTINTEN + (GPIO_TO_INDEX(gpio) << 2), GPIO_EXTINTEN_REG(gpio) | (1U << (gpio & EDGE_INDEX_MASK)));
...
static INT32 _CKGEN_GPIO_Intrq(INT32 i4GpioNum, INT32 *pfgSet)
{
    ...
    u4Val = (*pfgSet) ?
            ...
            (u4Val & ~(1U << (i4GpioNum & GPIO_INDEX_MASK)));
#if defined(CONFIG_ARCH_MT5883)	|| defined(CONFIG_ARCH_MT5891)	
	__ckgen_writel(_GPIO_EXTINTEN_REG(GPIO_TO_INDEX(i4GpioNum)), u4Val);
#else
    __ckgen_writel(REG_GPIO_EXTINTEN + (GPIO_TO_INDEX(i4GpioNum)<< 2), u4Val);
#endif
...
static irqreturn_t _GPIO_DedicatedIsr(int u2VctId, void *dev_id)
{
    ...
    {
    	#if defined(CONFIG_ARCH_MT5883) || defined(CONFIG_ARCH_MT5891)	
		u4Status[i] = __ckgen_readl(_GPIO_EXTINT_REG(i));
		#else
    	u4Status[i] = __ckgen_readl(REG_GPIO_EXTINT + 4*i);
		#endif
    }
    ...
    {
    	#if defined(CONFIG_ARCH_MT5883) || defined(CONFIG_ARCH_MT5891)	
		__ckgen_writel(_GPIO_EXTINT_REG(i), 0);
		#else
        __ckgen_writel((REG_GPIO_EXTINT + 4*i), 0);
		#endif
    }
...
static INT32 _CKGEN_GPIO_Init()
{
    ...
    {
    	#if defined(CONFIG_ARCH_MT5883) || defined(CONFIG_ARCH_MT5891)	
		__ckgen_writel(_GPIO_EXTINTEN_REG(i4Idx), 0);
		#else
		__ckgen_writel(REG_GPIO_EXTINTEN + (i4Idx << 2), 0);
		#endif
    }
    ...
    {
    	#if defined(CONFIG_ARCH_MT5883) || defined(CONFIG_ARCH_MT5891)	
        __ckgen_writel(_GPIO_EXTINT_REG(i4Idx), 0);
		#else
        __ckgen_writel(REG_GPIO_EXTINT + (i4Idx << 2), 0);
		#endif
    }
