...
void insert_stress_gpd(DEV_INT32 ep_num,USB_DIR dir, DEV_UINT8* buf, DEV_UINT32 count, DEV_UINT8 isHWO, DEV_UINT8 IOC);
...
void dev_prepare_gpd_short(DEV_INT32 num,USB_DIR dir,DEV_INT32 ep_num,DEV_UINT8* buf){
	...
	os_printk(K_ERR,"buf[%d] :%x\r\n",ep_num,(unsigned int)buf);
...
void dev_qmu_loopback_ext(DEV_INT32 ep_rx,DEV_INT32 ep_tx){
	...
	os_printk(K_INFO,"rreq->buf :%x\r\n",(unsigned int)rreq->buf);
	os_printk(K_INFO,"treq->buf :%x\r\n",(unsigned int)treq->buf);
	...
	dma_sync_single_for_device(NULL, mapping, g_dma_buffer_size, DMA_BIDIRECTIONAL);
	treq->dma_adr=rreq->dma_adr=(DEV_UINT8* )mapping;
	...
	mapping=(dma_addr_t)rreq->dma_adr;
	dma_sync_single_for_cpu(NULL,mapping,g_dma_buffer_size,DMA_BIDIRECTIONAL);
	dma_unmap_single(NULL, mapping, g_dma_buffer_size,DMA_BIDIRECTIONAL);
...
void dev_qmu_rx(DEV_INT32 ep_rx){
	...
	mapping=(dma_addr_t)req->dma_adr;
	dma_sync_single_for_cpu(NULL,mapping,g_dma_buffer_size,DMA_BIDIRECTIONAL);
	dma_unmap_single(NULL, mapping, g_dma_buffer_size,DMA_BIDIRECTIONAL);
 ...
 */
TGPD* prepare_tx_stress_gpd(TGPD* gpd, DEV_UINT8*pBuf, DEV_UINT32 data_length, DEV_UINT8 ep_num, DEV_UINT8 _is_bdp, DEV_UINT8 isHWO,DEV_UINT8 IOC){
	DEV_UINT32  offset;
	DEV_INT32 i,bd_num; 
...
void resume_gpd_hwo(USB_DIR dir,DEV_INT32 Q_num){
	...
	if(dir == USB_RX){
		...
		gpd_current = (TGPD* )gpd_phys_to_virt(gpd_current,dir,Q_num);
		
		if(dir == USB_RX){
			gpd_current = (TGPD* )((DEV_UINT8*)gpd_current+STRESS_IOC_TH*Rx_gpd_IOC[Q_num]*sizeof(TGPD));
		}
		if(dir == USB_TX){
			gpd_current = (TGPD* )((DEV_UINT8*)gpd_current+STRESS_IOC_TH*Rx_gpd_IOC[Q_num]*sizeof(TGPD));
			gpd_current = (TGPD* )((DEV_UINT8*)gpd_current+STRESS_IOC_TH*Rx_gpd_IOC[Q_num]*AT_GPD_EXT_LEN);
		}
	...
	for(i=0;i<(STRESS_IOC_TH);i++){
		...
		gpd_current++;
		if(dir == USB_TX){
			gpd_current = gpd_current+AT_GPD_EXT_LEN;
		}
	}
...
void set_gpd_hwo(USB_DIR dir,DEV_INT32 Q_num){
	...
	for(i=0;i<(MAX_GPD_NUM);i++){
		if(dir == USB_RX){
			...
			TGPD_SET_BUF_LEN(gpd_current, 0);
			gpd_current_tx++;
			gpd_current_tx = gpd_current_tx+AT_GPD_EXT_LEN;
 ...
 * @args - arg1: gpd address, arg2: data buffer address, arg3: data length, arg4: ep number, arg5: with bd or not, arg6: write hwo bit or not,  arg7: write ioc bit or not
 */
TGPD* prepare_rx_stress_gpd(TGPD*gpd, DEV_UINT8*pBuf, DEV_UINT32 data_len, DEV_UINT8 ep_num, DEV_UINT8 _is_bdp, DEV_UINT8 isHWO, DEV_UINT8 IOC){
	DEV_UINT32  offset;
	DEV_INT32 i,bd_num; 
...
void mu3d_hal_restart_qmu_no_flush(DEV_INT32 Q_num, USB_DIR dir, DEV_INT8 method){
	...
	if(dir == USB_TX){		  
		...
		gpd_current = (TGPD*) gpd_phys_to_virt(gpd_current,USB_TX,Q_num);
...
//    TGPD*gpd_free; 
	...
	if(g_run_stress){
		...
		if(gpd==gpd_current){
			...
			gpd=(TGPD*)gpd_phys_to_virt(gpd,USB_RX,ep_num);
    	}    
...
//     TGPD*gpd_free; 
	...
	if(g_run_stress){
		...
		gpd_current = (TGPD*)gpd_phys_to_virt(gpd_current,USB_TX,ep_num);
    	...
    	while(gpd!=gpd_current && !TGPD_IS_FLAGS_HWO(gpd)){
		 os_printk(K_DEBUG,"Tx gpd %x info { HWO %d, BPD %d, Next_GPD %x , DataBuffer %x, BufferLen %d, Endpoint %d}\r\n", 
			...
			gpd=TGPD_GET_NEXT(gpd);	
			gpd=(TGPD*)gpd_phys_to_virt(gpd,USB_TX,ep_num);
   		 }
...
//	DEV_UINT8 IOC;
	...
	if(wQmuVal & TXQ_LENERR_INT){
		os_printk(K_ALET,"Tx %d length error!\r\n", i); 
		os_printk(K_ALET,"\r\n"); 
		//while(1);
	}
...
irqreturn_t u3d_inter_handler(int irq, void *dev_id){
    ...
    DEV_UINT32 dwLtssmValue=0;
    DEV_UINT32 ep_num;
//    DEV_UINT32 dwDisableTimer;
    DEV_UINT32 dwTemp;
	...
	os_printk(K_DEBUG,"dwLtssmValue : %x\n",dwLtssmValue);
	os_printk(K_DEBUG,"dwIntrUsbValue : %x\n",dwIntrUsbValue);
	os_printk(K_DEBUG,"dwDmaIntrValue : %x\n",dwDmaIntrValue);
	os_printk(K_DEBUG,"wIntrTxValue : %x\n",wIntrTxValue);
	os_printk(K_DEBUG,"wIntrRxValue : %x\n",wIntrRxValue);
	os_printk(K_DEBUG,"wIntrQMUValue : %x\n",wIntrQMUValue);
	os_printk(K_DEBUG,"wIntrQMUDoneValue : %x\n",wIntrQMUDoneValue);
	os_printk(K_DEBUG,"dwLinkIntValue : %x\n",dwLinkIntValue);	
	os_printk(K_DEBUG,"U3D_EP0CSR : %x\n",os_readl(U3D_EP0CSR));
	os_printk(K_DEBUG,"USB_EPIER : %x\n",os_readl(U3D_EPIER));
	os_printk(K_DEBUG,"USB_EPISR :%x\n",os_readl(U3D_EPISR));

	if (!(dwLtssmValue | dwIntrUsbValue | dwDmaIntrValue | wIntrTxValue | wIntrRxValue | wIntrQMUValue | wIntrQMUDoneValue | dwLinkIntValue))
		return IRQ_NONE;
	os_printk(K_INFO, "[START INTERRUPT]\n");
    ...
    if(wIntrTxValue){
    	for(ep_num = 1; ep_num <= TX_FIFO_NUM; ep_num++){
          	if(wIntrTxValue & (1<<ep_num)){
				os_printk(K_ERR,"wIntrTxValue :%x\n",wIntrTxValue);
