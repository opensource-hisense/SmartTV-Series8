...
void u3d_ep0en(void)
{
	...
	temp |= ((g_ep0_mode == PIO_MODE) ? 0 : EP0_DMAREQEN);
	//temp |= EP0_DMAREQEN;	
...
void dev_lpm_config(LPM_INFO *lpm_info){
	...
	os_printk(K_ALET,"dev_lpm_config\n");
	...
	else
	{
		...
		else
		{
			...
			if(req->actual >= req->count)
			{
				...
				dma_unmap_single(NULL, (dma_addr_t)req->dma_adr, USB_BUF_SIZE,DMA_BIDIRECTIONAL);
...
//	DEV_UINT8 *ptr2;
	...
	else
	{
		...
		g_dma_debug = (DEV_UINT8*) mapping;
		dma_sync_single_for_device(NULL, mapping, USB_BUF_SIZE, DMA_BIDIRECTIONAL);
		req->dma_adr = (DEV_UINT8*) mapping;
		u3d_config_dma0(0, USB_RX, (DEV_INT32) req->dma_adr, req->currentCount);
...
void u3d_epx_handler(DEV_INT32 ep_num, USB_DIR dir){
	...
	if(dir == USB_TX){   
		...
		if(req->actual == req->count){
			if((!req->complete) && (req->count == maxp)){
			...
			if((!req->complete) && (req->actual == req->count)&&(!(req->count % maxp))){
				mu3d_hal_write_fifo_burst(ep_num,0,bp, maxp);
			}
		}
