...
DEV_INT32 U3D_Phy_Cfg_Cmd(DEV_INT32 argc, DEV_INT8 **argv){
	...
	if (u3phy->phy_version == 0xd60802a && latch_val == 0)
	{
		latch_val = 1;
	}
...
//	struct USB_REQ *req;
	...
	unsigned long flags;
    USB_SPEED speed;	
	DEV_UINT32 status = RET_SUCCESS,i,stop_count_1,stop_count_2;//,temp1,temp2,temp3,temp4,temp5,temp6;
		...
		if(u3d_req_valid()){
			...
			switch(u3d_command()){
				...
				case SINGLE:					
					...
					os_printk(K_INFO,"sg_info->transfer_length 0x%x\n",sg_info->transfer_length);
					os_printk(K_INFO,"sg_info->gpd_buf_size 0x%x\n",sg_info->gpd_buf_size);
					os_printk(K_INFO,"sg_info->bd_buf_size 0x%x\n",sg_info->bd_buf_size);
