...
PBD get_bd(USB_DIR dir,DEV_UINT32 num){
	...
	if(dir==USB_RX){
		...
		os_printk(K_DEBUG,"(Rx_bd_List[%llx].pNext : 0x%p\n", num,(DEV_UINT32)(Rx_bd_List[num].pNext));		
...
void init_gpd_list(USB_DIR dir,int num,PGPD ptr,PGPD io_ptr,DEV_UINT32 size){
	...
	if(dir==USB_RX){
		...
		os_printk(K_DEBUG,"Rx_gpd_List[%d].pStart : 0x%p\n", num,(ulong)Rx_gpd_List[num].pStart);
		os_printk(K_DEBUG,"Rx_gpd_List[%d].pNext  : 0x%p\n", num,(ulong)Rx_gpd_List[num].pNext);
		os_printk(K_DEBUG,"Rx_gpd_List[%d].pEnd   : 0x%p\n", num,(ulong)Rx_gpd_List[num].pEnd);
		os_printk(K_DEBUG,"Rx_gpd_Offset[%d]   : 0x%p\n", num,Rx_gpd_Offset[num]);
		os_printk(K_DEBUG,"phy : 0x%p\n",(ulong)io_ptr);
		...
		os_printk(K_DEBUG,"Tx_gpd_List[%d].pStart : 0x%p\n", num,(ulong)Tx_gpd_List[num].pStart);
		os_printk(K_DEBUG,"Tx_gpd_List[%d].pNext  : 0x%p\n", num,(ulong)Tx_gpd_List[num].pNext);
		os_printk(K_DEBUG,"Tx_gpd_List[%d].pEnd   : 0x%p\n", num,(ulong)Tx_gpd_List[num].pEnd);
		os_printk(K_DEBUG,"Tx_gpd_Offset[%d]   : 0x%p\n", num,Tx_gpd_Offset[num]);
		os_printk(K_DEBUG,"phy : 0x%p\n",(ulong)io_ptr);
		os_printk(K_DEBUG,"phy end : 0x%p\n",(ulong)io_ptr+size);
		os_printk(K_ERR,"io_ptr : 0x%p\n",(ulong)io_ptr);
...
//		ptr = (TGPD*)os_mem_alloc(size);
		...
		Rx_gpd_end[i]=ptr;
		os_printk(K_ERR,"ALLOC RX GPD End [%d] Virtual Mem@0x%p\n", i, (DEV_UINT32)Rx_gpd_end[i]);
		os_memset(Rx_gpd_end[i], 0 , sizeof(TGPD));
		TGPD_CLR_FLAGS_HWO(Rx_gpd_end[i]);    
	    Rx_gpd_head[i]=Rx_gpd_last[i]=Rx_gpd_end[i];
		os_printk(K_ERR,"RQSAR[%d] : 0x%p\n", i, (ulong)mu3d_hal_gpd_virt_to_phys(Rx_gpd_end[i],USB_RX,i));
		size = sizeof(TGPD);
		size += AT_GPD_EXT_LEN;
		size *=MAX_GPD_NUM;
        ptr = dma_alloc_coherent(NULL, size, &dma,
...
//		ptr = (TGPD*)os_mem_alloc(size);
	    ...
	    Tx_gpd_end[i]= ptr;
	    os_printk(K_ERR,"ALLOC TX GPD End [%d] Virtual Mem@0x%p\n", i, (DEV_UINT32)Tx_gpd_end[i]);
		...
		os_printk(K_ERR,"TQSAR[%d] : 0x%p\n", i, (ulong)mu3d_hal_gpd_virt_to_phys(Tx_gpd_end[i],USB_TX,i));
		size = (sizeof(TBD));
		size *= MAX_BD_NUM;
...
void mu3d_hal_init_qmu(void){
	...
	for(i=1; i<=MAX_QMU_EP; i++){
		QCR|=QMU_RX_EN(i);
		QCR|=QMU_TX_EN(i);
		os_writel((void*)USB_QMU_RQSAR(i), (ulong)mu3d_hal_gpd_virt_to_phys(Rx_gpd_head[i],USB_RX,i));
		os_writel((void*)USB_QMU_TQSAR(i), (ulong)mu3d_hal_gpd_virt_to_phys(Tx_gpd_head[i],USB_TX,i));
...
TGPD* mu3d_hal_prepare_tx_gpd(TGPD* gpd, DEV_UINT8*pBuf, DEV_UINT32 data_length, DEV_UINT8 ep_num, DEV_UINT8 _is_bdp, DEV_UINT8 isHWO,DEV_UINT8 ioc, DEV_UINT8 bps,DEV_UINT8 zlp){
	...
	os_printk(K_INFO,"Malloc Tx 01 (GPD+EXT) (Tx_gpd_end) : 0x%llu\r\n", (DEV_UINT32)Tx_gpd_end[ep_num]);
...
TGPD* mu3d_hal_prepare_rx_gpd(TGPD*gpd, DEV_UINT8*pBuf, DEV_UINT32 data_len, DEV_UINT8 ep_num, DEV_UINT8 _is_bdp, DEV_UINT8 isHWO, DEV_UINT8 ioc, DEV_UINT8 bps, DEV_UINT32 cMaxPacketSize){
	...
	os_printk(K_INFO,"Rx Next GPD 0x%p, current BUF_LEN 0x%x\n",(DEV_UINT32)Rx_gpd_end[ep_num],TBD_GET_DataBUF_LEN(bd));
    ...
    if(dir == USB_TX){    
		...
		os_writel(U3D_TQERRIESR0, QMU_TX_LEN_ERR(Q_num)|QMU_TX_CS_ERR(Q_num));
      	Temp = os_readl(USB_QMU_TQCSR(Q_num));
       	os_printk(K_DEBUG,"USB_QMU_TQCSR:0x%p\n", Temp);  
		...
		os_writel(U3D_RQERRIESR0, (ulong)(QMU_RX_LEN_ERR(Q_num)|QMU_RX_CS_ERR(Q_num)));
		os_writel(U3D_RQERRIESR1, (ulong)(QMU_RX_EP_ERR(Q_num)|QMU_RX_ZLP_ERR(Q_num)));
...
//    DEV_UINT16 wVal,i; 
    ...
    if(dir == USB_TX){     
		...
		gpd_current = (TGPD*)(ulong)(os_readl(USB_QMU_TQCPR(Q_num)));
		if(!gpd_current){
			gpd_current = (TGPD*)(ulong)(os_readl(USB_QMU_TQSAR(Q_num)));
		}
		...
		free_gpd(dir,Q_num);
		os_writel((void*)USB_QMU_TQSAR(Q_num), (ulong)mu3d_hal_gpd_virt_to_phys(Tx_gpd_last[Q_num],USB_TX,Q_num));
		...
		gpd_current = (TGPD*)(ulong)(os_readl(USB_QMU_RQCPR(Q_num)));
		if(!gpd_current){
			gpd_current = (TGPD*)(ulong)(os_readl(USB_QMU_RQSAR(Q_num)));
		}
		os_printk(K_CRIT,"gpd_current(P) %p\r\n", (ulong)gpd_current);
		...
		free_gpd(dir,Q_num);
		os_writel((void*)USB_QMU_RQSAR(Q_num), (ulong)mu3d_hal_gpd_virt_to_phys(Rx_gpd_end[Q_num],USB_RX,Q_num));
		os_printk(K_ERR,"USB_QMU_RQSAR %p\r\n", os_readl(USB_QMU_RQSAR(Q_num)));
		req->complete=true;
