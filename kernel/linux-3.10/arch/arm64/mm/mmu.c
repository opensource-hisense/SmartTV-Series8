...
static void __init alloc_init_pte_old(pmd_t *pmd, unsigned long addr,
...
static void __init alloc_init_pmd_old(pud_t *pud, unsigned long addr,
		unsigned long end, phys_addr_t phys, unsigned int type)
{
	...
	do {
		...
		if (((addr | next | phys) & ~SECTION_MASK) == 0) {
			...
			alloc_init_pte_old(pmd, addr, next, __phys_to_pfn(phys), type);
		phys += next - addr;
	} while (pmd++, addr = next, addr != end);
}

static void __init alloc_init_pud_old(pgd_t *pgd, unsigned long addr,
		unsigned long end, unsigned long phys, unsigned int type)
{
	...
	do {
		next = pud_addr_end(addr, end);
		alloc_init_pmd_old(pud, addr, next, phys, type);
		phys += next - addr;
	} while (pud++, addr = next, addr != end);
}
 ...
 * mapping specified by 'md'.
 */
static void __init create_mapping_old(struct map_desc *io_desc)
{
	unsigned long addr, length, end, next;
	pgd_t *pgd;
	unsigned long virt = io_desc->virtual;
	phys_addr_t phys = __pfn_to_phys(io_desc->pfn);
	phys_addr_t size = io_desc->length;
	...
	do {
		next = pgd_addr_end(addr, end);
		alloc_init_pud_old(pgd, addr, next, phys, io_desc->type);
		phys += next - addr;
	} while (pgd++, addr = next, addr != end);
}
...
void iotable_init(struct map_desc *io_desc, int nr)
{
	...
	for (md = io_desc; nr; md++, nr--) {
		//create_mapping(__pfn_to_phys(md->pfn), md->virtual, md->length);
		create_mapping_old(md);
