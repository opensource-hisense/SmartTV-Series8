...
#if defined(CONFIG_MTK_PARTITION)
int test_part_mtkraw (block_dev_desc_t *dev_desc)
{
    /* TBD */
    if (dev_desc->type == MTD_DEV_TYPE_EMMC) {
        return (0);
    }
    else {
        return (-1);
...
int get_partition_info_mtkraw (block_dev_desc_t *dev_desc, int part, disk_partition_t * info)
{
	...
	struct part_info *pr_dst_part = NULL;

    dev = device_find(dev_desc->type, dev_desc->dev);
    if (dev) {
        pr_dst_part = mtd_part_info(dev, part);
    }
    else {
        return (-1);
    }
	...
	strncpy((char*)info->name, pr_dst_part->name, strlen(pr_dst_part->name));
	info->size = (ulong)(pr_dst_part->size >> SECTOR_BITS);
	info->start = (ulong)(pr_dst_part->offset >> SECTOR_BITS);
    ...
    defined(CONFIG_MTK_PARTITION)

void init_part (block_dev_desc_t * dev_desc)
{
#ifdef CONFIG_MTK_PARTITION
					...
					, disk_partition_t *info)
{
    ...
    case PART_TEYP_MTKRAW:
        if (get_partition_info_mtkraw(dev_desc,part,info) == 0) {
            PRINTF ("## Valid MTK raw partition found ##\n");
            return (0);
        }
        break;
#endif
...
#ifdef CONFIG_MTK_PARTITION
    case PART_TEYP_MTKRAW:
        PRINTF ("## Testing for valid MTK partition ##\n");
...
# error nor CONFIG_EFI_PARTITION nor CONFIG_MTK_PARTITION configured!
#endif
