 ...
 *
 *   $Id: //DTV/MP_BR/DTV_X_IDTV1501_1538_4_001_46_007/apollo/linux_core/uboot/u-boot-2011.12/common/cmd_mtdparts.c#1 $
...
#define SIZE64_REMAINING        0xFFFFFFFFFFFFFFFF
...
#define OFFSET64_NOT_SPECIFIED  0xFFFFFFFFFFFFFFFF
...
char mtdparts_default[MTDPARTS_MAXLEN];
...
static u64 memsize_parse64 (const char *const ptr, const char **retptr)
{
...
/**
 * Performs sanity check for supplied EMMC flash partition. Table of existing
 * NAND flash devices is searched and partition device is located. Alignment
 * with the granularity of nand erasesize is verified.
 *
 * @param id of the parent device
 ...
 * @return 0 if partition is valid, 1 otherwise
 */
static int part_validate_emmc(struct mtdids *id, struct part_info *part)
{
#if defined(CC_EMMC_BOOT)
    /* info for EMMC chips */
    struct mmc *emmc;

    emmc = &emmc_info[id->num];

    if ((part->offset) % emmc->read_bl_len) {
        printf("%s%d: partition (%s) start offset alignment incorrect\n",
                MTD_DEV_TYPE(id->type), id->num, part->name);
        return 1;
    }

    if (part->size % emmc->read_bl_len) {
        printf("%s%d: partition (%s) size alignment incorrect\n",
                MTD_DEV_TYPE(id->type), id->num, part->name);
        return 1;
    }
...
{
	if (part->size == SIZE64_REMAINING)
		part->size = id->size - part->offset;

	if (part->offset > id->size) {
		printf("%s: offset %08x%08x beyond flash size 0x%08x%08x\n", id->mtd_id,
            (u32)(part->offset>>32), (u32)(part->offset), (u32)(id->size>>32), (u32)(id->size));
		return 1;
	}
    ...
    else if (id->type == MTD_DEV_TYPE_EMMC)
        return part_validate_emmc(id, part);
    else
        debug("part_validate: invalid dev type\n");
...
static int part_parse(const char *const partdef, const char **ret, struct part_info **retpart)
{
	...
	*ret = NULL;
	...
	if (*p == '-') {
		...
		size = SIZE64_REMAINING;
		p++;
	} else {
		size = memsize_parse64(p, &p);
	...
	/* check for offset */
	offset = OFFSET64_NOT_SPECIFIED;
	if (*p == '@') {
		p++;
		offset = memsize_parse64(p, &p);
	}
	...
	if (name) {
		...
		sprintf(part->name, "0x%08lx%08lx@0x%08lx%08lx",
		        (u32)(size>>32), (u32)size, (u32)(offset>>32), (u32)offset);
		part->auto_name = 1;
	}
	...
	debug("+ partition: name %-22s size 0x%08x%08x offset 0x%08x%08x mask flags %d\n",
			part->name, (u32)(part->size>>32), (u32)(part->size),
			(u32)(part->offset>>32), (u32)(part->offset), part->mask_flags);
...
int mtd_device_validate(u8 type, u8 num, u64 *size)
{
	...
	else if (type == MTD_DEV_TYPE_EMMC)
	{
#if defined(CC_EMMC_BOOT)
		if (num < CFG_MAX_EMMC_DEVICE)
		{
			// ... now support 64bits size emmc
			*size = (u64)(emmc_info[num].capacity);
			return 0;
		}
		
		printf("no such EMMC device: %s%d (valid range 0 ... %d)\n",
				MTD_DEV_TYPE(type), num, CFG_MAX_EMMC_DEVICE - 1);
#else
		printf("support for EMMC devices not present\n");
#endif
	}
...
static int device_parse(const char *const mtd_dev, const char **ret, struct mtd_device **retdev)
{
	...
	*retdev = NULL;
	...
	while (p && (*p != '\0') && (*p != ';')) {
		...
		if (part->offset == OFFSET64_NOT_SPECIFIED)
			part->offset = offset;
		else
			offset = part->offset;
	...
	*dev_type = 0;
	if (strncmp(p, "nand", 4) == 0) {
		*dev_type = MTD_DEV_TYPE_NAND;
		p += 4;
    } else if (strncmp(p, "emmc", 4) == 0) {
        *dev_type = MTD_DEV_TYPE_EMMC;
        p += 4;
...
static void print_partition_table(void)
{
	...
	list_for_each(dentry, &devices) {
		...
		list_for_each(pentry, &dev->parts) {
			...
			printf("%2d: %-20s0x%08x%08x\t0x%08x%s\t0x%08x%08x\t%d\n",
					part_num, part->name, (u32)(part->size>>32), (u32)(part->size),
					net_size, size_note, (u32)(part->offset>>32), (u32)(part->offset),
					part->mask_flags);
		...
		list_for_each(pentry, &dev->parts) {
			...
			printf("%2d: %-20s0x%08x%08x\t0x%08x%08x\t%d\n",
					part_num, part->name,
					(u32)(part->size>>32), (u32)(part->size),
					(u32)(part->offset>>32), (u32)(part->offset),
					part->mask_flags);
	...
	if (current_mtd_dev) {
		...
		if (part) {
			printf("\nactive partition: %s%d,%d - (%s) 0x%08x%08x @ 0x%08x%08x\n",
					...
					current_mtd_dev->id->num, current_mtd_partnum, part->name,
					(u32)(part->size>>32), (u32)(part->size),
					(u32)(part->offset>>32), (u32)(part->offset));
		} else {
...
#define PART_ADD_DESC_MAXLEN 64
		...
		if (!strcmp(&argv[1][3], ".spread")) {
			...
			debug("increased %s to 0x%08x%08x bytes\n", p->name,
                  (u32)(p->size>>32), (u32)(p->size));
		}
#endif
