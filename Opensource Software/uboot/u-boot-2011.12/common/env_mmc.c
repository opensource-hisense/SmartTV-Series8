...
{
    #if defined(CONFIG_ENV_A_OFFSET) && defined(CONFIG_ENV_B_OFFSET)
        UINT8 boot_flag;
        ...
        if(boot_flag)
        {
            *env_addr = CONFIG_ENV_B_OFFSET;
        }
        else
        {
            *env_addr = CONFIG_ENV_A_OFFSET;
        }
    #else
...
void env_relocate_spec(void)
{
	...
	if (read_env(mmc, CONFIG_ENV_SIZE, offset, env_ptr))
		return set_default_env(NULL);
