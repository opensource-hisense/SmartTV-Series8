...
#include "../drivers/usb/pegasus.h"
extern int pegasus_init (struct usb_device *usbdev);
#endif

#ifdef CONFIG_DRIVER_RTL8150
#include "../drivers/usb/rtl8150.h"
extern int rtl8150_init (struct usb_device *usbdev);
#endif

#ifdef CONFIG_DRIVER_AX88772
#include "../drivers/usb/Ax88772.h"
extern int Ax88772_init(struct usb_device *usbdev);
extern int Ax88172_init(struct usb_device *usbdev);
#endif
...
void usb_display_config(struct usb_device *dev)
{
	...
	for(i=0;i<config->no_of_if;i++) {
	        if (USB_MAXINTERFACES > i)
                {
        		ifdesc= &config->if_desc[i];
        		usb_display_if_desc(ifdesc,dev);
        		for(ii=0;ii<ifdesc->no_of_ep;ii++) {
                            if (USB_MAXENDPOINTS > ii)
                            {
...
static int usb_scan_netdev(struct usb_device *dev)
{
#if (defined(CONFIG_DRIVER_RTL8150) || defined(CONFIG_DRIVER_AX88772) || defined(CONFIG_DRIVER_PEGASUSII))
              		
#ifdef CONFIG_DRIVER_PEGASUSII
	if (dev->descriptor.idVendor == VENDOR_ID_ADMTEK)
	{
	    pegasus_init(dev);
	    printf("ADM85xx device is found !\n");
	    return 1;
	}
#endif					
#ifdef CONFIG_DRIVER_RTL8150
	if ((dev->descriptor.idVendor == VENDOR_ID_REALTEK) &&
	    (dev->descriptor.idProduct == PRODUCT_ID_RTL8150))
	{
	    rtl8150_init(dev);
	    printf("Rtl8150 device is found !\n");
	    return 1;
	}
#endif
#ifdef CONFIG_DRIVER_AX88772
	if ((dev->descriptor.idVendor == VENDOR_ID_AX88772) &&
	      (dev->descriptor.idProduct == PRODUCT_ID_AX88772))
	{
	    Ax88772_init(dev);
	    printf("Ax88772 device is found !\n");
	    return 1;
	}

	if ((dev->descriptor.idVendor == VENDOR_ID_AX88772) &&
	      (dev->descriptor.idProduct == PRODUCT_ID_AX88172))
	{
	    Ax88172_init(dev);
	    printf("Ax88172 device is found !\n");
	    return 1;
	}
#endif
#endif
...
int usb_check_dev(int uboot_upgrade)
{
    int d;
    int i, NumOfPorts;
    struct usb_device *dev;

#if defined(CONFIG_ARCH_MT5881) || defined(CONFIG_ARCH_MT5883)
    NumOfPorts = 2;
	...
	/* For Wukong 5891, No USB2.0 Port2*/
	if (i == 2) {
	    continue;
	}
#endif
    ...
    for (d = 0; d < USB_MAX_DEVICE;d++) 
    {
        ...
        if (( usb_stor_curr_dev == -1) &&
            (dev) && 
            (dev->descriptor.bDeviceClass == 0x08) && (uboot_upgrade))
        {
...
int do_usb (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    ...
    if (strncmp(argv[1], "hub",  3) == 0){
        if ( (argc != 4)  || (0 != usb_hub_electrical_test(argv[2],argv[3])) )
        {
            printf("hub parameter error ! Usage:\n");
            printf("(1) usb hub se0nak n\n");
            printf("(2) usb hub testj n \n");
            printf("(3) usb hub testk n \n");
            printf("(4) usb hub testpkt n \n");
            printf("(5) usb hub slewrate n \n");
            printf("(6) usb hub jklevel n \n");            
        }		
        return 1;
    }
