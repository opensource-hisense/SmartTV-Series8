...
unsigned long usb_stor_read(int device, unsigned long blknr, lbaint_t blkcnt, void *buffer);
...
unsigned long usb_stor_read(int device, unsigned long blknr, lbaint_t blkcnt, void *buffer)
{
	...
	if (blkcnt == 0 || buffer==NULL )
	{
		printf("\nusb_stor_read: [blkcnt=%d][buffer=0x%x]\n",blkcnt,buffer);
		return 0;
	}

	device &= 0xff;

        if (USB_MAX_STOR_DEV <= device)
        {
            return 0;
        }
...
int usb_storage_probe(struct usb_device *dev, unsigned int ifnum,struct us_data *ss)
{
	 ...
	 */
        if (USB_MAXENDPOINTS <= iface->bNumEndpoints)
        {
            return 0;
        }
...
int usb_stor_get_info(struct usb_device *dev,struct us_data *ss,block_dev_desc_t *dev_desc)
{
	...
	memcpy((void *)&cap[0], (const void *)&usb_stor_buf[0], 8);	
