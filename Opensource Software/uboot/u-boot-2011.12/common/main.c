...
#ifdef NDEBUG
extern unsigned int UBOOT_EEPDTV_IsSupportCLI(void);
#endif
...
int abortboot(int bootdelay)
{
	int abort = 0;
		...
		if (UBOOT_EEPDTV_IsSupportCLI() == 0)
		{
			return 0;
		}
#endif
...
# endif	/* CONFIG_BOOT_RETRY_TIME */
...
#endif /* CONFIG_BOOTCOUNT_LIMIT */
		#ifdef CONFIG_USB_FASTBOOT
		if(BIM_CheckFastboot())
		{
		    char newstr[50] = {0}; 
			#define ETHERNET_BASE1	(0xF0032014)
			#define ETHERNET_BASE2	(0xF0032018)
		    sprintf(newstr, "%08x%08x", (*((volatile u32*)(ETHERNET_BASE1))),(*((volatile u32*)(ETHERNET_BASE2))));
			memcpy(surf_udc_device.serialno, newstr, sizeof(newstr)); 
    ...
    // replace bootcmd with recovery command
    if (BIM_IsRecoveryMode())
    {
        s = getenv ("recovery_cmd");
    }
    else
#endif
		s = getenv ("bootcmd");
...
#if !(CONFIG_NO_CONSOLE)	//skip bootdelay checking
	if (bootdelay >= 0 && s && !abortboot (bootdelay))
#endif		
	{
		char *s1;
		
		/* Check if we should enable the watchdog  */
		if ( ((s1 = getenv("watchdog")) != NULL) && (strcmp(s1,"yes") == 0) )
		{   
            watchdog_enable();
			printf("\nEnable Watchdog\n");
		}
