 ...
 */
void usb_scan_devices(int portnum);

int usb_hub_probe(struct usb_device *dev, int ifnum);
void usb_hub_reset(void);
extern int usb_lowlevel_init_with_port_num(int iPortNum);
...
// electrical test under hub downstream port 
int usb_hub_electrical_test(char *argv,char *argv1)
{
    char agrv_t1;
    ...
    if(strlen(argv1) == 2)
        {
        agrv_t1 = '1';
        if(strncmp(argv1, "11", 2) == 0)
            argv1 = "1";
        else if(strncmp(argv1, "12", 2) == 0)
            argv1 = "2";
        else if(strncmp(argv1, "13", 2) == 0)
            argv1 = "3";
        else if(strncmp(argv1, "14", 2) == 0)
            argv1 = "4";
        else
            {
        ...
        //printf("\033[0;34m the current parameter is %s %s \033[0m\n",argv1, &agrv_t1);

        printf("Enter Hub downstream port  Electrical Test mode...\n");
        
        if (strncmp(argv, "se0nak", 6) == 0){
            return usb_hub_sendse0nak(argv1);
        }

        if (strncmp(argv, "testj", 5) == 0){
            return usb_hub_sendtestj(argv1);
        }

        if (strncmp(argv, "testk", 5) == 0){
            return usb_hub_sendtestk(argv1);
        }

        if (strncmp(argv, "testpkt", 7) == 0){
            return usb_hub_sendtestpacket(argv1);
        }

        if (strncmp(argv, "slewrate", 8) == 0){
            return usb_hub_sendslewratepacket(&agrv_t1,argv1);
        }

        if (strncmp(argv, "jklevel", 7) == 0){
            return usb_hub_sendjklevelpacket(&agrv_t1,argv1);
        }
...
int usb_init(int iPortNum)
{
	...
	if(iPortNum == CONFIG_WIFI_PORT_NUMBER)
	{
		printf("disable scan wifi port %d.....\n", iPortNum);
		return -1;
	}
    #endif
	result = usb_lowlevel_init_with_port_num(iPortNum);
	/* if lowlevel init is OK, scan the bus for devices i.e. search HUBs and configure them */
	if(result==0) {
		printf("scanning bus for devices... ");
		running=1;
		usb_scan_devices(iPortNum);
		usb_started = 1;
		return 0;
	}
	else {
...
{
	int i,ii,b;
	struct usb_endpoint_descriptor rep;
	struct usb_endpoint_descriptor *ep;

	for(i=0; i<dev->config.bNumInterfaces;i++) {
		for(ii=0; ii<dev->config.if_desc[i].bNumEndpoints; ii++) {

                        if ((i >= USB_MAXINTERFACES) || (ii >= USB_MAXENDPOINTS))
                        {
                            return -1;
                        }          
		    
                        // Max Liao, 2008/12/25, fix endpoint alignment cause data abort problem.
			memcpy((void *)&rep, (const void *)&dev->config.if_desc[i].ep_desc[ii], 
		...
		switch(head->bDescriptorType) {
			case USB_DT_INTERFACE:
				...
				memcpy(&dev->config.if_desc[ifno],&buffer[index],buffer[index]); /* copy new desc */
                                printf("Interface-%d: bInterfaceClass=%d.\n", ifno, dev->config.if_desc[ifno].bInterfaceClass);
                                if (dev->descriptor.bDeviceClass == 0)
                                {
                                    dev->descriptor.bDeviceClass = dev->config.if_desc[ifno].bInterfaceClass;
                                }
...
//  MTK Notice: Max Liao, 2007/12/05.
//  ipod must wait 100 ms after set configuration.	
...
void usb_scan_devices(int portnum)
{
	...
	dev->portnum = portnum;
	usb_new_device(dev);
	printf("New USB Device found, index=%d.\n",dev_index);
...
void usb_hub_port_connect_change(struct usb_device *dev, int port)
{
	...
	usb->parent=dev;
    usb->portnum = usb->parent->portnum;           // add for uboot update fail through hub 
