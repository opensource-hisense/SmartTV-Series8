...
static void env_del_bootargs(char* oldstr, char* newstr)
{
    char *p1 = NULL, *p2 = NULL;
    char *bootargs = NULL;
...
int do_addboot (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	arg = argv[1];
	env_add_bootargs(arg,arg);
	saveenv();
	return 1;
}

int do_delboot (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
 ...
 */
    ...
    || (defined(CONFIG_CMD_ENV) && defined(CONFIG_CMD_EMMC)) \
    || (defined(CONFIG_CMD_ENV) && defined(CONFIG_CMD_ONENAND))) \
    && !defined(CFG_ENV_IS_NOWHERE))

extern int reloadenv(void);

int do_reloadenv (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
    extern char * env_name_spec;

    printf ("Reloading default environment to %s...\n", env_name_spec);

    return (reloadenv() ? 1 : 0);
}
#endif
...
U_BOOT_CMD(
    reloadenv, 1, 0,    do_reloadenv,
    "reloadenv  - reload default environment variables\n",
    NULL
);
#endif

U_BOOT_CMD(
    addboot, CFG_MAXARGS, 0,   do_addboot,
    "addboot  - append environment variables 'bootargs'\n",
    "value ...\n"
    "    - append 'value' to environment variable 'bootargs' ...'\n"
);
U_BOOT_CMD(
    delboot, CFG_MAXARGS, 0,   do_delboot,
    "delboot  - del environment variables 'bootargs'\n",
    "value ...\n"
    "    - del 'value' to environment variable 'bootargs' ...'\n"
);
