 ...
 *   kernel tree.
 *
 *   $Id: //DTV/MP_BR/DTV_X_IDTV1501_1538_4_001_46_007/apollo/linux_core/uboot/u-boot-2011.12/common/cmd_jffs2.c#1 $
	...
	*dev_type = 0;
	if (strncmp(p, "nand", 4) == 0) {
		...
		*dev_type = MTD_DEV_TYPE_ONENAND;
		...
		printf("incorrect device type in the %s\n", id);
		return 1;
	}

	if (!isdigit(*p)) {
		printf("incorrect device number in the %s\n", id);
		return 1;
	}
...
/**
 * Given partition name find corresponding number, size and offset.
 *
 * @param part_name string describing partition name
 * @param part_num verified partition number (output)
 * @param part_size verified partition size (output)
 * @param part_offset verified partition offset (output)
 * @return 0 on success, 1 otherwise
 */
int find_partition(const char *part_name, u8 *part_num, u64 *part_size, u64 *part_offset)
{
    struct list_head *dentry, *pentry;
    struct part_info *part;
    ...
    list_for_each(dentry, &devices) {
        ...
        list_for_each(pentry, &dev->parts) {
            ...
            {
                printf("Partition %s defined at mtdparts:\nID:%d, Offset:0x%08x, Size:0x%08x\n",
                    part->name, pnum, (unsigned int)part->offset, (unsigned int)part->size);
                *part_num = pnum;
                *part_size = part->size;
