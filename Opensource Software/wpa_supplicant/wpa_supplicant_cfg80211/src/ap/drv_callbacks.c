...
#ifdef DRIVER_TRIGGER_CHANNEL_SWITCH
	int i, j;
	for (i = 0; i < hapd->iface->num_hw_features; i++) {
		struct hostapd_hw_modes *mode = &hapd->iface->hw_features[i];
		for (j = 0; j < mode->num_channels; j++) {
			struct hostapd_channel_data *ch = &mode->channels[j];
			if (ch->freq == freq)
			{
				hostapd_logger(hapd, NULL, HOSTAPD_MODULE_IEEE80211,
                               		HOSTAPD_LEVEL_WARNING, "Found channel: %d[freq: %d] in mode: %d", ch->chan, freq, i+1);
				hapd->iface->conf->hw_mode = i+1;
				hapd->iface->conf->channel = ch->chan;
				goto FOUND;
			}	
		}
	}
FOUND:
	hostapd_select_hw_mode(hapd->iface);
#endif /* DRIVER_TRIGGER_CHANNEL_SWITCH */
...
#ifdef DRIVER_TRIGGER_CHANNEL_SWITCH
	hostapd_prepare_rates(hapd->iface, hapd->iface->current_mode);
	ieee802_11_set_beacon(hapd);
#endif /* DRIVER_TRIGGER_CHANNEL_SWITCH */
#endif /* NEED_AP_MLME */
}
