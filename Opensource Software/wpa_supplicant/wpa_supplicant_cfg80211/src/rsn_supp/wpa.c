					  ...
					  size_t key_data_len)
{
	...
	wpa_driver_nl80211_set_ptk(sm->ctx->ctx,(u8 *) &(sm->ptk), sm->pairwise_cipher!=WPA_CIPHER_TKIP?48:64);
...
#ifdef CONFIG_WOW_SUPPORT
	wpa_driver_nl80211_set_replay_counter(sm->ctx->ctx,key->replay_counter,WPA_REPLAY_COUNTER_LEN);
#endif /* CONFIG_WOW_SUPPORT */
