...
#define TDLS_MAX_WFD_IE_LEN 128
#define WFD_IE_H_LEN 6
#define WFD_OUI_LEN 4
u8 wfd_header[] = {0xDD, 0x00, 0x50, 0x6F, 0x9A, 0x0A};
u8 tdls_wpa2_only = 0;
...
struct wpa_tdls_peer {
	...
	u8 *wfdie;
	size_t wfdie_len;
	u8 wfd_subelem_device_info[6];
	u8 wfd_subelem_associated_bssid[6];
	u8 wfd_subelem_audio_formats[15];
	u8 wfd_subelem_vidio_formats[21];
	u8 wfd_subelem_3d_vidio_formats[17];
	u8 wfd_subelem_content_protection[1];
	u8 wfd_subelem_coupled_sink[7];
	u8 wfd_subelem_ext_capab[2];
	u8 wfd_subelem_local_ip_address[5];
	u8 *wfd_subelem_session_info;
	size_t wfd_subelem_session_info_len;
#endif
};


#ifdef CONFIG_WIFI_DISPLAY
int wpa_tdls_parse_wfd_ies(struct wpa_tdls_peer *peer)
{
	const u8 *pos, *end;
	int ret = 0;

	for (pos = peer->wfdie, end = pos + peer->wfdie_len; pos + 1 < end; pos += 3 + WPA_GET_BE16(pos + 1)) {
		if (*pos == WFD_SUBELEM_DEVICE_INFO) {
			os_memcpy(peer->wfd_subelem_device_info, pos + 3, 6);
		} else if (*pos == WFD_SUBELEM_ASSOCIATED_BSSID) {
			os_memcpy(peer->wfd_subelem_associated_bssid, pos + 3, 6);
		} else if (*pos == WFD_SUBELEM_AUDIO_FORMATS) {
			os_memcpy(peer->wfd_subelem_audio_formats, pos + 3, 15);
		} else if (*pos == WFD_SUBELEM_VIDEO_FORMATS) {
			os_memcpy(peer->wfd_subelem_vidio_formats, pos + 3, 21);
		} else if (*pos == WFD_SUBELEM_3D_VIDEO_FORMATS) {
			os_memcpy(peer->wfd_subelem_3d_vidio_formats, pos + 3, 17);
		} else if (*pos == WFD_SUBELEM_CONTENT_PROTECTION) {
			os_memcpy(peer->wfd_subelem_content_protection, pos + 3, 1);
		} else if (*pos == WFD_SUBELEM_COUPLED_SINK) {
			os_memcpy(peer->wfd_subelem_coupled_sink, pos + 3, 7);
		} else if (*pos == WFD_SUBELEM_EXT_CAPAB) {
			os_memcpy(peer->wfd_subelem_ext_capab, pos + 3, 2);
		} else if (*pos == WFD_SUBELEM_LOCAL_IP_ADDRESS) {
			os_memcpy(peer->wfd_subelem_local_ip_address, pos + 3, 5);
		} else if (*pos == WFD_SUBELEM_SESSION_INFO) {
			peer->wfd_subelem_session_info = pos;
			peer->wfd_subelem_session_info_len = WPA_GET_BE16(pos + 1);
		} else {
			wpa_printf(MSG_INFO, "TDLS: Unrecognized WFD IE subelement %d", *pos);
		}
	}
...
int wpa_tdls_wfd_init(struct wpa_supplicant *wpa_s)
{
	char tmp[30];
	char cmd_bssid[24] = {'1', ' ', '0', '0', '0', '6'};
	char cmd_ip[24] = {'8', ' ', '0', '0', '0', '5', '0', '1'};
	struct in_addr addr;

	if (wpa_s->wpa_state == WPA_COMPLETED)
	{
		wpa_snprintf_hex(cmd_bssid + 6, 16, wpa_s->bssid, ETH_ALEN);
		if (wifi_display_subelem_set(wpa_s->global, cmd_bssid) < 0)
		{
			wpa_printf(MSG_DEBUG, "TDLS: WFD init fail, write bssid to wfdie fail");
			return -1;
		}
	}
	else
	{
		wpa_printf(MSG_DEBUG, "TDLS: WFD init fail, not associated wpa_state=%d", wpa_s->wpa_state);
		return -1;
	}
	...
	{
		inet_aton(tmp, &addr);
		wpa_snprintf_hex(cmd_ip + 8, 14, &addr, sizeof(addr));
		if (wifi_display_subelem_set(wpa_s->global, cmd_ip) < 0)
		{
			wpa_printf(MSG_DEBUG, "TDLS: WFD init fail, write local ip to wfdie fail");
			return -1;
		}
	}
	else
	{
		wpa_printf(MSG_DEBUG, "TDLS: WFD init fail, l2 IP get fail");
		return -1;
	}

	if (wifi_display_set_wfd_dev_info(wpa_s->global, "wfd_preferredConnectivity", 1) < 0)
	{
		wpa_printf(MSG_DEBUG, "TDLS: WFD init fail, set preferred connectivity bit fail");
		return -1;
	}
...
int wpa_tdls_wfd_exit(struct wpa_supplicant *wpa_s)
{
	char cmd[3];
	...
	{
		wpa_printf(MSG_DEBUG, "TDLS: WFD exit fail, clear bssid for wfdie fail");
		return -1;
	}
	...
	if (wifi_display_subelem_set(wpa_s->global, cmd) < 0)
	{
		wpa_printf(MSG_DEBUG, "TDLS: WFD exit fail, clear local ip for wfdie fail");
		return -1;
	}

	if (wifi_display_set_wfd_dev_info(wpa_s->global, "wfd_preferredConnectivity", 0) < 0)
	{
		wpa_printf(MSG_DEBUG, "TDLS: WFD exit fail, clear preferred connectivity bit fail");
		return -1;
	}
			     ...
			     int initiator, const u8 *msg, size_t msg_len)
{
	...
	if (action_code == WLAN_TDLS_SETUP_CONFIRM ||
		...
		action_code == WLAN_TDLS_TUNNELED_PROBE_REQ ||
		action_code == WLAN_TDLS_TUNNELED_PROBE_RSP)
		return 0; /* No retries */
...
skip_ftie:
	 ...
	 */
#ifdef CONFIG_WIFI_DISPLAY
	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, TDLS_EVENT_PEER_TEARDOWN MACSTR, MAC2STR(src_addr));
#endif
	wpa_tdls_disable_peer_link(sm, peer);
	return 0;
}
				...
				struct wpa_tdls_peer *peer)
{
	...
	int status;
#ifdef CONFIG_WIFI_DISPLAY
	int write = 0, buf_size = TDLS_MAX_WFD_IE_LEN;
	u8 *pos_wfd_ie_len;
#endif /* CONFIG_WIFI_DISPLAY */
...
#ifdef CONFIG_WIFI_DISPLAY
	if (tdls_wpa2_only && sm->proto != WPA_PROTO_RSN)
	{
		/* it is not a proper place to change state, just for Miracast certification */
		sm->tdls_link_state = WFD_TDLS_WEAK_SECURITY;
		return -1;
	}
#endif /* CONFIG_WIFI_DISPLAY */
...
skip_ies:
...
#ifdef CONFIG_WIFI_DISPLAY
	os_memcpy(pos, wfd_header, WFD_IE_H_LEN);
	pos_wfd_ie_len = pos + 1;
	pos += WFD_IE_H_LEN;
	*pos_wfd_ie_len = WFD_OUI_LEN;

	/*(0)WFD_SUBELEM_DEVICE_INFO*/
	write = wifi_display_subelem_memcpy(sm->global, WFD_SUBELEM_DEVICE_INFO, pos, buf_size);
	if (write < 0)
	{
		os_free(rbuf);
		wpa_tdls_peer_free(sm, peer);
	...
	pos += write;
	*pos_wfd_ie_len += write;
	buf_size -= write;

	/*(1)WFD_SUBELEM_LOCAL_IP_ADDRESS*/
	write = wifi_display_subelem_memcpy(sm->global, WFD_SUBELEM_ASSOCIATED_BSSID, pos, buf_size);
	if (write < 0)
	{
		os_free(rbuf);
		wpa_tdls_peer_free(sm, peer);
	...
	*pos_wfd_ie_len += write;
	...
	/*(8)WFD_SUBELEM_LOCAL_IP_ADDRESS*/
	write = wifi_display_subelem_memcpy(sm->global, WFD_SUBELEM_LOCAL_IP_ADDRESS, pos, buf_size);
	if (write < 0)
	{
		os_free(rbuf);
		wpa_tdls_peer_free(sm, peer);
...
#ifdef CONFIG_WIFI_DISPLAY
int wpa_tdls_send_probe_request(struct wpa_sm *sm, const u8 *addr)
{
	int i, write = 0, buf_size;
	u8 wfd_ie_hex[TDLS_MAX_WFD_IE_LEN]; /*assume 128 is enough*/
	u8 *pos;
	//u8 wfd_header[] = {0xDD, 0x00, 0x50, 0x6F, 0x9A, 0x0A};
	struct wpa_global *global = sm->global;

	if (sm->tdls_disabled || !sm->tdls_supported)
		return -1;

	wpa_printf(MSG_INFO, "TDLS: Sending Probe Request to peer "
		   MACSTR, MAC2STR(addr));

	os_memcpy(wfd_ie_hex, wfd_header, WFD_IE_H_LEN);
	pos = wfd_ie_hex + WFD_IE_H_LEN;
	buf_size = sizeof(wfd_ie_hex) - WFD_IE_H_LEN;

	/*WFD probe request should include subelement 0, 1, 6, 7*/
	/*(0)WFD_SUBELEM_DEVICE_INFO*/
	write = wifi_display_subelem_memcpy(global, WFD_SUBELEM_DEVICE_INFO, pos, buf_size);
	if (write < 0)
		return write;
	pos += write;
	buf_size -= write;

	/*(1)WFD_SUBELEM_ASSOCIATED_BSSID*/
	write = wifi_display_subelem_memcpy(global, WFD_SUBELEM_ASSOCIATED_BSSID, pos, buf_size);
	if (write < 0)
		return write;
	pos += write;
	buf_size -= write;

	/*(6)WFD_SUBELEM_COUPLED_SINK*/
	write = wifi_display_subelem_memcpy(global, WFD_SUBELEM_COUPLED_SINK, pos, buf_size);
	if (write < 0)
		return write;
	pos += write;
	buf_size -= write;

	/*(7)WFD_SUBELEM_EXT_CAPAB*/
	write = wifi_display_subelem_memcpy(global, WFD_SUBELEM_EXT_CAPAB, pos, buf_size);
	if (write < 0)
		return write;
	pos += write;
	buf_size -= write;

	wfd_ie_hex[1] = pos - wfd_ie_hex - WFD_IE_H_LEN + WFD_OUI_LEN;

	return wpa_tdls_tpk_send(sm, addr, WLAN_TDLS_TUNNELED_PROBE_REQ, 1, 0, 0, 0, wfd_ie_hex, (pos - wfd_ie_hex));
}


int wpa_tdls_send_probe_response(struct wpa_sm *sm, const u8 *addr)
{
	int i, write = 0, buf_size;
	u8 wfd_ie_hex[TDLS_MAX_WFD_IE_LEN]; /*assume 128 is enough*/
	u8 *pos;
	...
	wpa_printf(MSG_INFO, "TDLS: Sending Probe Response to peer "
		   MACSTR, MAC2STR(addr));
	...
	/*WFD probe request should include subelement 0, 1, 6, 7, 9, 10*/
	/*(0)WFD_SUBELEM_DEVICE_INFO*/
	write = wifi_display_subelem_memcpy(global, WFD_SUBELEM_DEVICE_INFO, pos, buf_size);
	if (write < 0)
		return write;
	...
	/*(9)WFD_SUBELEM_EXT_CAPAB*/
	write = wifi_display_subelem_memcpy(global, WFD_SUBELEM_SESSION_INFO, pos, buf_size);
	if (write < 0)
		return write;
	pos += write;
	buf_size -= write;

	/*To do: no alternative MAC address defination*/
	/*(10)WFD_SUBELEM_EXT_CAPAB*/
	//write = wifi_display_subelem_memcpy(global, , pos, buf_size);
	//if (write < 0)
		//return write;
	//pos += write;
	//buf_size -= write;
	...
	return wpa_tdls_tpk_send(sm, addr, WLAN_TDLS_TUNNELED_PROBE_RSP, 1, 0, 0, 0, wfd_ie_hex, (pos - wfd_ie_hex));
}


static int wpa_tdls_parse_probe_response(struct wpa_sm *sm, const u8 *src_addr,
				   const u8 *buf, size_t len)
{
	char wfd_dev_info[13];
	u8 WFD_OUI[] = {0x50, 0x6F, 0x9A, 0x0A};

	wpa_printf(MSG_INFO, "TDLS parsing Tunneled Probe Response from: "
		   MACSTR " size: %d", MAC2STR(src_addr), (int)len);

	if (len < 6 + WFD_IE_H_LEN)
		return -1; /*too short*/

	if (os_memcmp(buf + 8, WFD_OUI, 4) != 0)
		return -1; /*without WFD IE*/

	if (buf[7] < 10)
	{
		wpa_printf(MSG_INFO, "TDLS parsing Tunneled Probe Response from: "
		   MACSTR " NO WFD sub Elements", MAC2STR(src_addr));
		return -1;
	}

	wpa_snprintf_hex(wfd_dev_info, sizeof(wfd_dev_info), buf + 6 + WFD_IE_H_LEN + 3, 6);

	wpa_msg(sm->ctx->msg_ctx, MSG_INFO, TDLS_EVENT_DEVICE_FOUND MACSTR " wfd_peer_dev_info=%s", MAC2STR(src_addr), wfd_dev_info);

	return 0;
}


static int wpa_tdls_process_tunneled_probe(struct wpa_sm *sm, const u8 *src_addr,
				   const u8 *buf, size_t len)
{
	const u8 *cpos;
	...
	wpa_printf(MSG_INFO, "TDLS receive Tunneled Probe Packet from: "
		   MACSTR " size: %d", MAC2STR(src_addr), (int)len);

	if (len < 1 + 1 + 3 + 1)
	{
		wpa_printf(MSG_DEBUG, "TDLS Tunneled Probe Packet is too short: "
			   "%d", (int) len);
		return -1;
	}

	cpos = buf;
	cpos += 1 /* pkt_type */ + 1 /* Category */;

	if (os_memcmp(cpos, WFA_OUI, 3) != 0)
	{
		wpa_printf(MSG_DEBUG, "TDLS Tunneled Probe Packet dump: wrong OUI %02X %02X %02X", cpos, cpos+1, cpos+2);
		return -1; /*not a WFA frame*/
	}
	cpos += 3 /* OUI */;

	if (*cpos == 4) /*Probe Request*/
	{
		wpa_printf(MSG_INFO, "TDLS receive Tunneled Probe Request");
		wpa_tdls_send_probe_response(sm, src_addr);
	}
	else if (*cpos == 5) /*Probe Response*/
	{
		wpa_printf(MSG_INFO, "TDLS receive Tunneled Probe Response");
		wpa_tdls_parse_probe_response(sm, src_addr, buf, len);
	}
...
static int copy_peer_wfdie(const struct wpa_eapol_ie_parse *kde,
			       struct wpa_tdls_peer *peer)
{
	if (!kde->wfdie) {
		wpa_printf(MSG_DEBUG, "TDLS: No WFDIE "
			   "received");
		return 0;
	}

	if (!peer->wfdie || peer->wfdie_len < kde->wfdie_len) {
		/* Need to allocate buffer to fit the new information */
		os_free(peer->wfdie);
		peer->wfdie = os_zalloc(kde->wfdie_len);
		if (peer->wfdie == NULL)
			return -1;
	}

	peer->wfdie_len= kde->wfdie_len;
	os_memcpy(peer->wfdie, kde->wfdie, peer->wfdie_len);
				   ...
				   const u8 *buf, size_t len)
{
	...
	if (tdls_wpa2_only && sm->proto != WPA_PROTO_RSN)
	{
		wpa_printf(MSG_INFO, "TDLS: Only WPA2 is allowed for WFD");
...
#ifdef CONFIG_WIFI_DISPLAY
	if (copy_peer_wfdie(&kde, peer) < 0)
		goto error;
	wpa_tdls_parse_wfd_ies(peer);
#endif /* CONFIG_WIFI_DISPLAY */
...
skip_rsn:
	...
	if (!peer->tpk_success) {
		...
		{
			sm->tdls_link_state = WFD_TDLS_LINKED;
			wpa_snprintf_hex(wfd_dev_info, sizeof(wfd_dev_info), peer->wfd_subelem_device_info, 6);
			wpa_snprintf_hex(wfd_local_ip, sizeof(wfd_local_ip), peer->wfd_subelem_local_ip_address, 5);

			os_memcpy(sm->tdls_dev_info, wfd_dev_info, sizeof(sm->tdls_dev_info));
			os_snprintf(sm->tdls_peer_ip, sizeof(sm->tdls_peer_ip), "%d.%d.%d.%d",
				peer->wfd_subelem_local_ip_address[0],
				peer->wfd_subelem_local_ip_address[1],
				peer->wfd_subelem_local_ip_address[2],
				peer->wfd_subelem_local_ip_address[3]);

			wpa_msg(sm->ctx->msg_ctx, MSG_INFO, TDLS_EVENT_PEER_CONNECTED MACSTR
				" wfd_peer_dev_info=%s" " wfd_peer_local_ip=%s"
				, MAC2STR(src_addr), wfd_dev_info, wfd_local_ip);
		}
#endif
	}
	return ret;
...
#ifdef CONFIG_WIFI_DISPLAY
	if (tf->category == WLAN_ACTION_VENDOR_SPECIFIC)
	{
		wpa_tdls_process_tunneled_probe(sm, src_addr, buf, len);
	}
	else
#endif
	{
...
int wpa_tdls_is_disabled(struct wpa_sm *sm)
{
	return sm->tdls_disabled;
}


int wpa_tdls_is_prohibited(struct wpa_sm *sm)
{
	return sm->tdls_prohibited;
}
...
char* wpa_tdls_peer_ip(struct wpa_sm *sm)
{
	return sm->tdls_peer_ip;
}

char* wpa_tdls_dev_info(struct wpa_sm *sm)
{
	return sm->tdls_dev_info;
}

int wpa_tdls_link_state(struct wpa_sm *sm)
{
	return sm->tdls_link_state;
}
