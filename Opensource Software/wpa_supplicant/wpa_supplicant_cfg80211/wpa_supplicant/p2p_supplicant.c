...
static void p2p_go_configured(void *ctx, void *data)
{
	...
	if (ssid && ssid->mode == WPAS_MODE_P2P_GO) {
		...
		wpas_notify_p2p_group_started(wpa_s, ssid, network_id, 0);
		wpas_p2p_cross_connect_setup(wpa_s);
		if(wpa_s->p2p_first_connection_timeout == 0)
		{
			wpas_p2p_set_group_idle_timeout(wpa_s);
		}
		else
		{
			...
			wpa_dbg(wpa_s, MSG_DEBUG, "P2P: Do not set p2p group idle time when p2p_group_formation timeout=%d(not equal 0)", wpa_s->p2p_first_connection_timeout);
		}
...
{
#ifdef CONFIG_CONSISTENT_P2P_IF_NAME
	os_snprintf(ifname, len, "p2p1");
#else
...
static void wpas_go_neg_req_rx(void *ctx, const u8 *src, u16 dev_passwd_id)
{
    ...
    if (dev && dev->info.wfd_subelems)
    {
        ...
        wpa_snprintf_hex(wfd_dev_info_hex, sizeof(wfd_dev_info_hex),
                    ...
                    WFD_DEV_INFO_SIZE);
        wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_WFD_INFO MACSTR
            " wfd_dev_info=0x%s", MAC2STR(src), wfd_dev_info_hex);
...
int wpas_get_acs_rank(void *ctx, u8 *buf, int buf_len)
{
	struct wpa_supplicant *wpa_s = ctx;
    return wpa_drv_get_acs_rank_list(wpa_s, buf, buf_len);
}
#endif
...
#endif  /*CONFIG_SUPPORT_GET_WFD_ASSOCBSSID_IN_CONN_MSG*/
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_WFD_INFO MACSTR
            " wfd_dev_info=0x%s", MAC2STR(peer), wfd_dev_info_hex);
				     ...
				     int op_freq)
{
...
#ifdef CONFIG_NOTIFY_INVITE_REQ_EVENT
	...
	wpa_printf(MSG_ERROR, "P2P: Remove pending group while p2p invitation procedure fail(status=%d).", status);
	wpas_p2p_remove_pending_group_interface(wpa_s);	
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
...
#if defined (CONFIG_SET_5G_HIGHER_PRIORITY)
    p2p.channel_rank = wpas_get_acs_rank;
#endif
	...
	if (wpa_s->conf->p2p_oper_reg_class &&
        ...
        p2p.op_channel = CONFIG_FIXED_P2P_OP_CHANNEL;
        wpa_printf(MSG_DEBUG, "P2P: Fix Operating Channel to %d", p2p.op_channel);
#else
		wpa_printf(MSG_DEBUG, "P2P: Random operating channel: "
			   "%d:%d", p2p.op_reg_class, p2p.op_channel);
#endif /* CONFIG_FIXED_P2P_OP_CHANNEL */
	}
		     ...
		     int ht40, int vht)
{
	...
	struct p2p_device* dev = p2p_get_device(wpa_s->global->p2p, peer_addr);
	if(dev != NULL)
	{	
		if(dev->peer_connect_recorded == 1)
		{
			if(dev->go_intent_recored == 1)
			{
				if(dev->go_intent == (P2P_MAX_GO_INTENT - 1))
				{
					go_intent = 15;
					wpa_printf(MSG_DEBUG, "dynamicly change the go_intent to 15 since peer go_intent<15");
				}
				else
				{			
					wpa_printf(MSG_DEBUG, "peer go_intent=%d, use default go_intent", dev->go_intent);
				}
				dev->go_intent_recored = 0;
			}
			else
			{			
				wpa_printf(MSG_DEBUG, "peer go_intent not recorded, use default go_intent");
			}
			dev->peer_connect_recorded = 0;		
		}
		else
		{
			wpa_printf(MSG_DEBUG, "peer p2p_connect request not receive, use default go_intent");
		}
		
	}
	else
	{
		wpa_printf(MSG_DEBUG, "unknown peer device, so do not change our go_intent dynamicly");
	}
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_LISTEN_CHANNEL) {
		...
		if (wpa_s->conf->p2p_listen_reg_class &&
			...
			channel = CONFIG_FIXED_P2P_LISTEN_CHANNEL;
			channel_forced = CONFIG_FIXED_P2P_LISTEN_CHANNEL;
			wpa_printf(MSG_DEBUG, "P2P: Fix Listen Channel to %d", channel);
#endif
		}
		wpa_printf(MSG_DEBUG, "P2P: %s: Listen Channel = %d\n", __FUNCTION__, channel);
	...
	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_OPER_CHANNEL) {
		...
		if (wpa_s->conf->p2p_oper_reg_class &&
			...
			cfg_op_channel = 0;
#ifdef CONFIG_FIXED_P2P_OP_CHANNEL
            op_channel = CONFIG_FIXED_P2P_OP_CHANNEL;
            wpa_printf(MSG_DEBUG, "P2P: Fix Operation Channel to %d", op_channel);
#endif
		}
        wpa_printf(MSG_DEBUG, "P2P: %s: Operation Channel = %d\n", __FUNCTION__, op_channel);
...
#ifdef CONFIG_SUPPORT_SET_OP_FREQ

	if(wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_OPER_FREQ)
	{
		u8 channel_buf[64] = {0x00};
		int channel_len = 0;
		if (wpa_s->conf->p2p_oper_freq == 2 || wpa_s->conf->p2p_oper_freq == 5)
		{
			channel_len = wpa_drv_get_acs_rank_list(wpa_s, channel_buf, (int)sizeof(channel_buf));
			if(channel_len)
			{
				wpa_printf(MSG_DEBUG, "Get ACS_RANK_LIST len=%d ", channel_len);
				wpa_hexdump(MSG_DEBUG, "ACS list:", channel_buf, channel_len);
			}
		}
		p2p_set_oper_freq(p2p, &(wpa_s->conf->p2p_oper_freq), &(wpa_s->conf->p2p_oper_reg_class),
							&(wpa_s->conf->p2p_oper_channel), channel_buf, channel_len);
	}

	wpa_printf(MSG_DEBUG, "wpa_s->conf->p2p_oper_freq = %d, wpa_s->conf->p2p_oper_reg_class = %d,"
					"wpa_s->conf->p2p_oper_channel = %d", wpa_s->conf->p2p_oper_freq,
		wpa_s->conf->p2p_oper_reg_class, wpa_s->conf->p2p_oper_channel);
#endif
#ifdef CONFIG_SUPPORT_SET_GO_INTENT
 	if (wpa_s->conf->changed_parameters & CFG_CHANGED_P2P_GO_INTENT) 
		 p2p_set_go_intent(p2p, wpa_s->conf->p2p_go_intent);
#endif
