...
void wpa_debug_print_timestamp(void)
{
	...
	if (out_file) {
		...
		time(&timep);
		tmp = localtime(&timep);
		fprintf(out_file, "date %d:%d:%d.%06u : ", tmp->tm_hour, tmp->tm_min, tmp->tm_sec, (unsigned int )tv.usec);
	} else
#endif /* CONFIG_DEBUG_FILE */
...
#ifdef CONFIG_SYSTEM_TIMESTAMP
	struct os_time tv;
	os_get_time(&tv);
	wpa_printf(level, "%ld.%06u: %s%s",(long) tv.sec, (unsigned int) tv.usec, prefix, buf);
#else
