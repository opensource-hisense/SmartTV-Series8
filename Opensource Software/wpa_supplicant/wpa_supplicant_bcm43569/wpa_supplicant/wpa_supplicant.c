...
#if defined(CONFIG_P2P) && defined(CONFIG_ANDROID_LOG) && \
    ( defined(CONFIG_P2P_LISTEN_CH_FOLLOW_AP) || defined(CONFIG_P2P_OPERATION_CH_FOLLOW_AP) )
    ...
    if ((os_strncmp(wpa_s->ifname, "wlan0", 5) == 0) && wpa_s->global->p2p)
    {
        ...
        if (state == WPA_COMPLETED && old_state != WPA_COMPLETED)
        {
            if (wpa_s->assoc_freq)
            {
                ...
                {
                	wpa_printf(MSG_DEBUG, "assoc_freq[%d] to channel [%d]\n",
                               wpa_s->assoc_freq, new_channel);
#ifdef CONFIG_P2P_LISTEN_CH_FOLLOW_AP
					if(!p2p_set_listen_channel(p2p, reg_class, new_channel, 1))
					{
						wpa_printf(MSG_DEBUG, "Set P2P Listen Channel to AP Channel (%d-ch%d)\n",
                               wpa_s->assoc_freq, new_channel);
					}
#else
					if (1 == new_channel || 6 == new_channel || 11 == new_channel)
					{
						if (!p2p_set_listen_channel(p2p, reg_class, new_channel, 1))
						{
							wpa_printf(MSG_DEBUG, "Set P2P Listen Channel to AP Channel (%d-ch%d)\n",
...
#endif
#ifdef CONFIG_P2P_OPERATION_CH_FOLLOW_AP
