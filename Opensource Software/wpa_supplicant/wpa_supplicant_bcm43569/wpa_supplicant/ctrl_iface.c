					    ...
					    char *buf, size_t buflen)
{
	...
	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		...
		if (wpa_s->assoc_freq){
			...
			if(wpa_s->assoc_freq > 2412 && wpa_s->assoc_freq < 2484 ){
				channel = (wpa_s->assoc_freq - 2412)/5 + 1;
			}
			else if(wpa_s->assoc_freq > 5180 && wpa_s->assoc_freq < 5825 ) {
				channel = (wpa_s->assoc_freq - 5180)/5 + 36;
			}
		...
		if (wpa_s->current_bss){
			u8  level = wpa_s->current_bss->level;
...
static int p2p_get_working_mode(struct wpa_supplicant *wpa_s,char *buf, size_t buflen)
{
            ...
            ssid->p2p_group)
    {
        ...
        {
        	freq = ssid->frequency;
            ret = os_snprintf(buf, buflen, "p2p_type=1\noper_freq=%d\n",freq);
        }
        else if (wpa_s->current_ssid->mode == WPAS_MODE_INFRA)
        {
            freq = wpa_s->current_bss ? wpa_s->current_bss->freq :
                   (int) wpa_s->assoc_freq;
            ret = os_snprintf(buf, buflen, "p2p_type=2\noper_freq=%d\n",freq);
        }
    }
	else
	{
        ret = os_snprintf(buf, buflen, "p2p_type=0\noper_freq=0\n");
	}
	return ret;
}
...
{
    wpa_printf(MSG_ERROR, "CTRL_IFACE P2P DISCONNECT '%s'", cmd);

    if (NULL == wpa_s) {

        wpa_printf(MSG_ERROR, "CTRL_IFACE P2P DISCONNECT wpa_s is NULL");
        return -1;
    }

	wpa_dbg(wpa_s, MSG_ERROR, "CTRL_IFACE P2P DISCONNECT '%s'", cmd);
    ...
    /*GC*/
	wpas_p2p_group_remove(wpa_s, wpa_s->ifname ); /* TODO*/
    }
	return 0;
}
#endif
			   ...
			   char *reply, int reply_size, int *reply_len)
{
	...
	if (params) {
		...
		if (pos){
			...
			os_memcpy(&(wpa_s->params_hidden_ssid),&scan_params,sizeof(scan_params));
					     }
       }			
...
{
	char *reply;
#ifdef DYNAMIC_BSS_RESULT_BUFFER
	int reply_size = 4096;
#else
...
#ifdef DYNAMIC_BSS_RESULT_BUFFER
	/* Adjust replay buffer for accommodating more BSSs (At least three houndreds) */
