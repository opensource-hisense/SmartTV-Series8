				      ...
				      const struct wpa_eapol_key *key)
{
	...
	if (wpa_sm_set_key(sm, WPA_ALG_KCK, sm->bssid, 0, 0, 0, 0,
			   (u8 *) sm->ptk.kck, 16) < 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: Failed to set KCK to the "
			"driver (alg=%d keylen=%d bssid=" MACSTR ")",
			alg, keylen, MAC2STR(sm->bssid));
		//return -1;
	}
	if (wpa_sm_set_key(sm, WPA_ALG_KEK, sm->bssid, 0, 0, 0, 0,
			   (u8 *) sm->ptk.kek, 16) < 0) {
		wpa_msg(sm->ctx->msg_ctx, MSG_WARNING,
			"WPA: Failed to set KEK to the "
			"driver (alg=%d keylen=%d bssid=" MACSTR ")",
			alg, keylen, MAC2STR(sm->bssid));
		//return -1;
	}
