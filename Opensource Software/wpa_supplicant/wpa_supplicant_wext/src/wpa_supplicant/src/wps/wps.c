...
/**
 * wps_get_rf_band - Get rf_band from WPS IE
 * @msg: WPS IE contents from Beacon or Probe Response frame
 * Returns: Pointer to rf_band or %NULL if not included
 *
 * The returned pointer is to the msg contents and it remains valid only as
 * long as the msg buffer is valid.
 */
const u8 * wps_get_rf_band(const struct wpabuf *msg)
{
	struct wps_parse_attr attr;

	if (wps_parse_msg(msg, &attr) < 0)
		return NULL;
	return attr.rf_bands;
}
...
#ifdef CONFIG_WPS_CONFIG_BY_CONF
	methods = wps ->config_methods;
	if(methods == 0)
		methods = 0x2388;
#endif
	wpa_printf(MSG_DEBUG, "WPS: Config_method = %x", methods);
	if (wps_build_version(ie) ||
	    wps_build_req_type(ie, req_type) ||
