...
struct wpa_driver_ops {
	...
	int (*p2p_connect)(void *priv, const u8 *peer_addr, int wps_method,
			   ...
			   unsigned int force_freq, int persistent_group, const u8 *pin);
	...
	int (*p2p_display_pin)(void *priv, unsigned int generated_pin);
	/**
	...
	int (*get_driver_version)(void *priv, char *ver);
	int (*get_p2p_association_table)(void *priv,struct _P2P_STA_ASSOC_LIST *AssocList);
	int (*get_p2p_operation_channel)(void *priv,char *OperationChannel);	
	int (*set_wpa_supplicant_support)(void *priv, u8 enable);
	int (*get_status)(void *priv, u8 *quailty, u8 *level, u8 *noise);
	int (*get_bitrate)(void *priv, u32 *bitrate);
	int (*get_associated_ap_info)(void *priv, char *name, u8 *channel, s32 *bitrate);
	int (*get_channel_status_info)(void *priv, u8 *channel); 
	int (*set_p2p_force_go)(void *priv, u8 enable, struct p2p_go_neg_results *params);
	int (*p2p_start_pin)(void *priv, const char *pin);
	int (*set_p2p_go_ssid)(void *priv, const char *ssid, const char *wpapsk);
	int (*p2p_start_pbc)(void *priv, u8 *enable);
	int (*p2p_del_client)(void *priv, char *addr);
	int (*set_usb_suspend)(void *priv, u32 params);
	int (*set_multi_channel_enable)(void *priv, u8 enable);
	int (*p2p_set_customized_cmd)(void *priv, char *cmdstr, char *data, u32 len);
	int (*p2p_get_customized_cmd)(void *priv, char *cmdstr, char *data, u32 len);
	int (*get_802_11_statistics)(void *priv, unsigned long *pFailedCount, unsigned long *pRetryCount);
	int (*set_p2p_auto_accept)(void *priv, u8 enable);
	int (*p2p_wps_cancel)(void *priv);
	int (*set_p2p_check_peer_channel)(void *priv, u8 mode);
	int (*set_p2p_client_ageout)(void *priv, const u8 *addr, unsigned int ageout);
	int (*p2p_disconnect)(void *priv);
	#ifdef CONFIG_QUERY_P2P_CONNECT_STATE
	int (*get_p2p_connect_state)(void *priv, int* ConnectState);
	#endif
};
...
enum wpa_event_type {
	...
	EVENT_P2P_INVITE_REQUEST,
	EVENT_P2P_SD_REQUEST,
	EVENT_P2P_SD_RESPONSE,

	/* for driver report P2P Connected / DisConnected event */
	EVENT_P2P_CONNECTED,
	EVENT_P2P_DISCONNECTED,
	EVENT_P2P_WPS_COMPLETED,

	EVENT_P2P_LEGACY_CONNECTED,
	EVENT_P2P_LEGACY_DISCONNECTED,
	EVENT_P2P_AP_STA_CONNECTED,
	EVENT_P2P_AP_STA_DISCONNECTED,
	EVENT_P2P_DEVICE_TABLE_ITEM_DELETE,
	EVENT_P2P_GO_NEGO_FAIL_INTENT,
	EVENT_P2P_GO_NEGO_FAIL_RECV_RESP,
	EVENT_P2P_GO_NEGO_FAIL_RECV_CONFIRM,
	EVENT_P2P_WPS_FAIL,
	EVENT_P2P_WPS_TIMEOUT,
	EVENT_P2P_WPS_2MINS_TIMEOUT,
	EVENT_P2P_WPS_PBC_SESSION_OVERLAP,
	EVENT_P2P_WPS_SEND_M2D,
	EVENT_P2P_WPS_RECEIVE_NACK,
	EVENT_P2P_STOP_CONNECT,
	EVENT_P2P_DEAUTH_AP,
	/**
...
typedef	struct _WFD_COUPLED_SINK_INFO
{
#ifndef RT_BIG_ENDIAN
	u8 CoupledStat:2;					
	u8 Rsvd:6;
	u8 coupled_peer_addr[6];
#else
	u8 coupled_peer_addr[6];
	u8 Rsvd:6;
	u8 CoupledStat:2;					
#endif
} __attribute__ ((packed)) WFD_COUPLED_SINK_INFO, *PWFD_COUPLED_SINK_INFO;
...
union wpa_event_data {
	...
	struct _p2p_all_info
	{
		...
		u8 group_capab;
		char rssi;
		u16 supp_config_methods;//provision req will need.
		u8 ClientFlag;//connected report will need.
		u16 dev_passwd_id;
	...
	struct p2p_ap_sta_connected {
		u8 addr[6];
		u8 dev_addr[6];
#ifdef CONFIG_HE_SUPPORT
		u8 bIsGo;
		u8 bInsPerTable;
#ifdef CONFIG_WFD
		u16	rtsp_port;
#endif /* CONFIG_WFD */
#endif /* CONFIG_HE_SUPPORT */
	} p2p_ap_sta_connected;
