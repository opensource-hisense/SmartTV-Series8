...
struct p2p_config {
...
#ifdef CONFIG_WFD
	/**
	 * wfdEnable - Enable/Disable WFD function
	 */
	u8 wfdEnable;
	...
	/**
	 * sourceCoup - Coupled sink operation supported by WFD source
	 */
	u8 sourceCoup;

	/**
	 * sinkCoup - Coupled sink operation supported by WFD sink
	 */	
	u8 sinkCoup;

	/**
	 * sessionAvail - Available for WFD session
	 */	
	u8 sessionAvail;

	/**
	 * rtspPort - TCP port at which the device listens for RTSP messages
	 */		
	int rtspPort;

	/**
	 * maxThroughput - Maximum average throughput capability of the WFD Device represented in multiples of 1Mbps
	 */		
	int maxThroughput;

	/**
	 * contentProtect - Content Protection (HDCP2.x) capability of the WFD Device
	 */     
	int contentProtect;
