...
struct p2p_device * p2p_driver_create_device(struct p2p_data *p2p,
					     const u8 *addr)
{
	return p2p_create_device(p2p, addr);
}
...
#ifdef CONFIG_WFD
int p2p_set_wfd_enable(struct p2p_data *p2p, u8 enable)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD: Set WFD Enable: "
		"wfdEnable %u", enable);

	if (enable != 0 && enable != 1)
		return -1;
	else
		p2p->cfg->wfdEnable= enable;

	return 0;
}

int p2p_set_device_type(struct p2p_data *p2p, u8 type)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD: Set Device Type: "
		"devType %u", type);

	if (type < 0 || 3 < type) 
		return -1;
	else
		p2p->cfg->devType = type;

	return 0;
}

int p2p_set_source_coupled(struct p2p_data *p2p, u8 coupled)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD: Set Source Coupled: "
		"sourceCoup %u", coupled);

	if (coupled != 0 && coupled != 1)
		return -1;
	else
		p2p->cfg->sourceCoup= coupled;

	return 0;
}

int p2p_set_sink_coupled(struct p2p_data *p2p, u8 coupled)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD: Set Sink Coupled: "
		"sinkCoup %u", coupled);

	if (coupled != 0 && coupled != 1)
		return -1;
	else
		p2p->cfg->sinkCoup = coupled;

	return 0;
}

int p2p_set_session_available(struct p2p_data *p2p, u8 avail)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD: Set Session Available: "
		"sessionAvail %u", avail);

	if (avail != 0 && avail != 1)
		return -1;
	else
		p2p->cfg->sessionAvail= avail;

	return 0;
}

int p2p_set_rtsp_port(struct p2p_data *p2p, u16 port)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD: Set RTSP Port: "
		"rtspPort %u", port);

	if ( port < 0 || 65535 < port)
		return -1;
	else
		p2p->cfg->rtspPort = port;

	return 0;
}

int p2p_set_max_throughput(struct p2p_data *p2p, u16 throughput)
{
	wpa_msg(p2p->cfg->msg_ctx, MSG_DEBUG, "WFD: Set Max. Throughput: "
		"maxThroughput %u", throughput);

	if (throughput < 0)
		return -1;
	else
		p2p->cfg->maxThroughput= throughput;
