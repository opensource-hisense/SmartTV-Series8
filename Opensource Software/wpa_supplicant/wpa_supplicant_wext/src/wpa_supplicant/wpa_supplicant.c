...
#ifdef CONFIG_HE_SUPPORT /* Special event request for HE */
			      ...
			      struct wpa_bss *bss, struct wpa_ssid *ssid)
{
	...
	if (bss) {
		 ...
		 */
		wpa_msg(wpa_s, MSG_INFO, "ssid->ssid == NULL ");
		wpa_s->scan_req = 2;
		wpa_s->reassociate = 1;
		wpa_supplicant_req_scan(wpa_s, 0, 0);
		return;
...
#ifndef ANDROID
	old_ssid = wpa_s->current_ssid;//Cartet synced version6
#endif
...
struct wpa_ssid * wpa_supplicant_get_ssid(struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_USER_SPACE_MLME) {
		...
		wpa_printf(MSG_DEBUG,
			   "##### %s, Could not read SSID from MLME",
			   __func__);
		...
		if (res < 0) {
			...
			wpa_printf(MSG_DEBUG,
                           "##### %s, Could not read SSID from driver",
                           __func__);
	...
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_USER_SPACE_MLME)
		...
		wpa_msg(wpa_s, MSG_WARNING, "Could not read BSSID from "
			"driver");
		wpa_printf(MSG_DEBUG,
			   "##### %s, Could not read BSSID from driver",
			   __func__);
		return NULL;
	}
