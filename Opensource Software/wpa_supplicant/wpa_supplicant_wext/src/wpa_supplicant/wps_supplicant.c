...
static void wpas_wps_security_workaround_force(struct wpa_supplicant *wpa_s,
					 struct wpa_ssid *ssid,
					 const struct wps_credential *cred)
{
	/*
	 ...
	 * actually supports stronger security and select that if the client
	 * has support for it, too.
	 * Some AP chang SSID after WPS success, Force add CCMP & RSN, 
	 * It will be check when real connect scan in wpa_supplicant.c wpa_supplicant_set_suites.
	 */

	if (!(ssid->pairwise_cipher & WPA_CIPHER_CCMP) &&
	    (ssid->pairwise_cipher & WPA_CIPHER_TKIP)) {
		wpa_printf(MSG_DEBUG, "WPS: Force Add CCMP into the credential ");
		if (wpa_s->conf->ap_scan == 1)
			ssid->pairwise_cipher |= WPA_CIPHER_CCMP;
		else
			ssid->pairwise_cipher = WPA_CIPHER_CCMP;
				   ...
				   const struct wps_credential *cred)
{
	...
	wpas_wps_security_workaround_force(wpa_s, ssid, cred);
...
static void wpa_supplicant_wps_event_success(struct wpa_supplicant *wpa_s)
{
	...
	//Carter synced--->
	if (wpa_s->driver->set_wpa_supplicant_support) {
		wpa_s->driver->set_wpa_supplicant_support(wpa_s->drv_priv, 0x01);
	}
#if 0
		//Carter synced<---
#ifdef CONFIG_P2P
			     ...
			     struct wpa_ssid *selected)
{
	...
	//Carter synced--->
	if (wpa_s->driver->set_wpa_supplicant_support) {
		wpa_s->driver->set_wpa_supplicant_support(wpa_s->drv_priv, 0x81);
	}
	//Carter synced<---
...
#ifdef CONFIG_P2P		
		if(wpa_s->driver->p2p_wps_cancel)
		    return wpa_s->driver->p2p_wps_cancel(wpa_s->drv_priv);
		else
#endif		
...
int wpas_p2p_wps_cancel(struct wpa_supplicant *wpa_s)
{
#ifdef CONFIG_AP
	if (wpa_s->ap_iface) {
		wpa_printf(MSG_DEBUG, "WPS: Cancelling in P2P GO mode");
		if(wpa_s->driver->p2p_wps_cancel)
		    return wpa_s->driver->p2p_wps_cancel(wpa_s->drv_priv);
		else
			return wpa_supplicant_ap_wps_cancel(wpa_s);
...
#ifdef CONFIG_P2P
	else   //Cancel WPS when GC role 
	{
		wpa_printf(MSG_DEBUG, "WPS: Cancelling in P2P GC mode");
		if(wpa_s->driver->p2p_wps_cancel)
		    return wpa_s->driver->p2p_wps_cancel(wpa_s->drv_priv);
		else
		{
		    wpa_printf(MSG_ERROR, "No wps cancel function in wext");			
		    return 0;
		}
	}
#endif	
...
/*check same UUID and support 5G also*/
//+++
		if (wpas_wps_scan_pbc_overlap_is_dualband_G(wpa_s, bss, ssid)) 
		{
			wpa_printf(MSG_DEBUG, "   skip - WPS AP "
				   "there is a 5G with same UUID AP with active PBC, Ignore 2.4G");
			wpabuf_free(wps_ie);
			return 0;
		}
//---
...
int wpas_wps_scan_pbc_overlap_is_dualband_G(struct wpa_supplicant *wpa_s,
//			      struct wpa_bss *selected, struct wpa_ssid *ssid)
			      struct wpa_scan_res *scan_res_bss, struct wpa_ssid *ssid)
{
	const u8 *sel_uuid, *uuid;
	struct wpabuf *wps_ie;
	int ret = 0;
	struct wpa_bss *bss;
	const u8 *sel_rf_band,*rf_band;

	if (!eap_is_wps_pbc_enrollee(&ssid->eap))
		return 0;

	wpa_printf(MSG_DEBUG, "WPS: Check whether PBC session overlap is dualband "
		   " in scan results; selected BSSID " MACSTR,
		   MAC2STR(scan_res_bss->bssid));

	/* Make sure that only one AP is in active PBC mode */
//	wps_ie = wpa_bss_get_vendor_ie_multi(selected, WPS_IE_VENDOR_TYPE);
	wps_ie = wpa_scan_get_vendor_ie_multi(scan_res_bss, WPS_IE_VENDOR_TYPE);
	if (wps_ie) {
		sel_uuid = wps_get_uuid_e(wps_ie);
//+++		
		sel_rf_band = wps_get_rf_band(wps_ie);
//---
	...
	dl_list_for_each(bss, &wpa_s->bss, struct wpa_bss, list) {
		struct wpabuf *ie;
//		if (bss->bssid == selected)
		if(os_memcmp(bss->bssid, scan_res_bss->bssid, ETH_ALEN) == 0)
			continue;
...
//+++
		rf_band = wps_get_rf_band(ie);
//---
		wpa_hexdump(MSG_DEBUG, "WPS: UUID of the other BSS",
			    uuid, UUID_LEN);
		if (sel_uuid && uuid && os_memcmp(sel_uuid, uuid, UUID_LEN) == 0) 		
		 // UUID is not null, and same UUID, if 2.4G ignore, 5G will be select later.
		{
			if(rf_band && sel_rf_band)
			{
				if ((*rf_band == WPS_RF_50GHZ) && (*sel_rf_band == WPS_RF_24GHZ))
				{
					ret = 1;
					wpa_printf(MSG_DEBUG, "WPS: this one is 2.4G and there is another BSS in 5G with active PBC mode");
				}	
				else if((scan_res_bss->freq <=2484) && (*rf_band == (WPS_RF_50GHZ | WPS_RF_24GHZ)))
				{
					ret = 1;
					wpa_printf(MSG_DEBUG, "WPS: freq compare, there is another active PBC mode");
				}
			}
			else
			{
				wpa_printf(MSG_DEBUG, "WPS: rf_band is NULL ?!");			
			}
			wpa_msg(wpa_s, MSG_INFO, "WPS: PBC dual band same UUID overlap detected: "
				MACSTR " and " MACSTR,
				MAC2STR(scan_res_bss->bssid),
				MAC2STR(bss->bssid));
			wpabuf_free(ie);
