...
#include "blacklist.h" //Carter synced
#include "driver_i.h"
...
#endif /* CONFIG_TDLS */
#ifdef CONFIG_P2P
    } else if (os_strcasecmp(cmd, "p2p_del_client") == 0) {
        wpa_printf(MSG_ERROR, "p2p_del_client, value=[%s]", value);
        if (wpa_s->driver->p2p_del_client) {
            if (wpa_s->driver->p2p_del_client(wpa_s->drv_priv, value) < 0)
                ret = -1;
        } 
    } else if (os_strcasecmp(cmd, "p2p_persistent_enable") == 0) {
        int enable = atoi(value);
        wpa_printf(MSG_ERROR, "p2p_persistent_enable, enable=[%d]", enable);
        if (wpa_s->driver->p2p_set_customized_cmd) {
            if (wpa_s->driver->p2p_set_customized_cmd(wpa_s->drv_priv, cmd, &enable, sizeof(int)) < 0)
                ret = -1;
        } 
	} else if (os_strcasecmp(cmd, "p2p_auto_accept") == 0) {
		int enable = atoi(value);
		wpa_printf(MSG_ERROR, "p2p_auto_accept=%d", enable);
		if (wpa_s->driver->set_p2p_auto_accept) {
			if (wpa_s->driver->set_p2p_auto_accept(wpa_s->drv_priv, enable) < 0)
				ret = -1;
		} 
	} else if (os_strcasecmp(cmd, "p2p_check_peer_channel") == 0) {
		int mode = atoi(value);
		wpa_printf(MSG_ERROR, "p2p_check_peer_channel=%d", mode);
		if (wpa_s->driver->set_p2p_check_peer_channel) {
			if (wpa_s->driver->set_p2p_check_peer_channel(wpa_s->drv_priv, mode) < 0)
				ret = -1;
		} 
#endif /*CONFIG_P2P */
    /* Usb-suspend */
	} else if (os_strcasecmp(cmd, "usb_suspend") == 0) {
    if (wpa_s->driver->set_usb_suspend) {
        int suspend = atoi(value);
        wpa_s->driver->set_usb_suspend(wpa_s->drv_priv, suspend);
        }
	} else if (os_strcasecmp(cmd, "multi_channel_enable") == 0) {
    if (wpa_s->driver->set_multi_channel_enable) {
        u8 enable = atoi(value);
        wpa_s->driver->set_multi_channel_enable(wpa_s->drv_priv, enable);
        }
    /* End of USB-suspend */
	} else {
		value[-1] = '=';
					 ...
					 char *cmd, char *buf, size_t buflen)
{
	...
	if (os_strcmp(cmd, "version") == 0) {
		...
		if (res < 0 || (unsigned int) res >= buflen)
			return -1;
		return res;
	} else if (os_strcmp(cmd, "p2p_per_tab") == 0) {
        PRT_P2P_PERSISTENT_ENTRY pP2pPerTab = NULL;
        pP2pPerTab = (PRT_P2P_PERSISTENT_ENTRY)os_malloc(sizeof(RT_P2P_PERSISTENT_ENTRY)*MAX_P2P_TABLE_SIZE);
        if (!pP2pPerTab) {
            wpa_printf(MSG_ERROR, "%s: pP2pPerTab allocate fail", __FUNCTION__);
            res = -1;
        } else {
            os_memset(pP2pPerTab, 0, sizeof(RT_P2P_PERSISTENT_ENTRY)*MAX_P2P_TABLE_SIZE);
            if (wpa_s->driver->p2p_get_customized_cmd) {
                res = wpa_s->driver->p2p_get_customized_cmd(wpa_s->drv_priv, "p2p_per_tab", 
                            (char *)pP2pPerTab, sizeof(RT_P2P_PERSISTENT_ENTRY)*MAX_P2P_TABLE_SIZE);
                res = 0;
                if (res < 0)
                    res = -1;
                else {
                    if (buflen < (sizeof(RT_P2P_PERSISTENT_ENTRY)*MAX_P2P_TABLE_SIZE)) {
                        wpa_printf(MSG_ERROR, "%s: reply buffer size not enough! (current=%d, required=%d)", 
                                    __FUNCTION__, buflen, (sizeof(RT_P2P_PERSISTENT_ENTRY)*MAX_P2P_TABLE_SIZE));
                        res = -1;
                    } else {
                        int idx = 0;
                        int found = 0;
                        os_memcpy(buf, pP2pPerTab, (sizeof(RT_P2P_PERSISTENT_ENTRY)*MAX_P2P_TABLE_SIZE));
                        res = (sizeof(RT_P2P_PERSISTENT_ENTRY)*MAX_P2P_TABLE_SIZE);
                        wpa_printf(MSG_DEBUG, "-----[Persistent Group Table]-----");
                        /* print the persistent table for debug */
                        for (idx = 0; idx< MAX_P2P_TABLE_SIZE; idx++){
                            if (!pP2pPerTab[idx].bValid)
                                continue;
                            found ++;
                            wpa_printf(MSG_DEBUG, "Idx=[%d],MyRule=%d,Addr=[" MACSTR "]", 
                                idx, pP2pPerTab[idx].MyRule, MAC2STR(pP2pPerTab[idx].Addr));
                            wpa_printf(MSG_DEBUG, "\t SSID=[%s]", pP2pPerTab[idx].Profile.SSID.Ssid);
                            wpa_printf(MSG_DEBUG, "\t AuthType=[%d]", pP2pPerTab[idx].Profile.AuthType);
                            wpa_printf(MSG_DEBUG, "\t EncrType=[%d]", pP2pPerTab[idx].Profile.EncrType);
                            wpa_printf(MSG_DEBUG, "\t EncrType=[%d]", pP2pPerTab[idx].Profile.EncrType);
                            wpa_printf(MSG_DEBUG, "\t KeyLen=[%d]", pP2pPerTab[idx].Profile.KeyLength);
                            wpa_printf(MSG_DEBUG, "\t KeyIndex=[%d]", pP2pPerTab[idx].Profile.KeyIndex);
                            wpa_printf(MSG_DEBUG, "\t APMacAddr=[" MACSTR "]", MAC2STR(pP2pPerTab[idx].Profile.MacAddr));
                        }
                        wpa_printf(MSG_DEBUG, "-----[%d Entries in table]-----", found);
                    }
                }
            } else {
                res = -1;
					     ...
					     char *cmd)
{
	...
	wpa_printf(MSG_ERROR, "wpa_s->ifname '%s'",
				   wpa_s->ifname);

	if (os_strcmp(wpa_s->ifname, "p2p0") == 0) {
		u8 enable[] = "1";

		wpa_printf(MSG_DEBUG, "CTRL_IFACE WPS_PBC: cmd is NULL, it shall call from P2P_PBC operation");		
		return wpa_drv_start_pbc(wpa_s, &enable);
	}
					     ...
					     size_t buflen)
{
	...
	if (pin) {
		*pin++ = '\0';
		wpa_printf(MSG_ERROR, "##### CTRL_IFACE WPS_PIN: '%s', len of pin = %d",
			pin, os_strlen(pin));
	}
	...
	if (os_strcmp(wpa_s->ifname, "p2p0") == 0) {
		...
		ret = wpa_drv_start_pin(wpa_s, pin);//Carter add for control Ralink Driver directly.
					    ...
					    char *buf, size_t buflen)
{
	...
	if (wpa_s->wpa_state >= WPA_ASSOCIATED) {
		...
		if (wpa_s->driver->get_status) {
			u8 quality = 0;
			u8  level = 0;
			u8 noise = 0;
			if (wpa_s->driver->get_status(wpa_s->drv_priv, &quality, &level, &noise) == 0 ) {
				ret = os_snprintf(pos, end - pos, "Link Quality=%d/100\n",
						  quality);
				if (ret < 0 || ret >= end - pos)
					return pos - buf;
				pos += ret;

				ret = os_snprintf(pos, end - pos, "signal_level=%hhd\n",
						  level);
				if (ret < 0 || ret >= end - pos)
					return pos - buf;
				pos += ret;

				ret = os_snprintf(pos, end - pos, "noise_level=%hhd\n",
						  noise);
				if (ret < 0 || ret >= end - pos)
					return pos - buf;
				pos += ret;
		...
		if ( wpa_s->driver->get_bitrate ) {
			s32 bitrate = 0;
			if ( wpa_s->driver->get_bitrate(wpa_s->drv_priv, &bitrate) == 0 ) {
				ret = os_snprintf(pos, end - pos, "current_rate=%d Mb/s\n",
						  bitrate);
				if (ret < 0 || ret >= end - pos)
					return pos - buf;
				pos += ret;
		...
		if ( wpa_s->driver->get_associated_ap_info ) {
			s32 bitrate = 0;
			char name[IFNAMSIZ];
			u8 channel;
			if ( wpa_s->driver->get_associated_ap_info(wpa_s->drv_priv, name, &channel, &bitrate) == 0 ) {
				ret = os_snprintf(pos, end - pos, "rate=%d Mb/s\n",
						  bitrate);
				if (ret < 0 || ret >= end - pos)
					return pos - buf;
				pos += ret;

				ret = os_snprintf(pos, end - pos, "protocol_caps=802.%s\n",
						  name);
				if (ret < 0 || ret >= end - pos)
					return pos - buf;
				pos += ret;
		...
		if ( wpa_s->driver->get_channel_status_info ) {
			s32 bitrate = 0;																			
			u8 channel; 																				
			if ( wpa_s->driver->get_channel_status_info(wpa_s->drv_priv, &channel) == 0 ) { 					
				ret = os_snprintf(pos, end - pos, "ap_channel=%d\n",														  
					channel); 																
				if (ret < 0 || ret >= end - pos)																	
					return pos - buf;																	
				pos += ret; 																																									
...
#if 1
#ifdef CONFIG_P2P
	char if_addr[ETH_ALEN];
	enum wpa_driver_if_type type = WPA_IF_P2P_GO;

	if (wpa_s->driver->if_add)
		wpa_s->driver->if_add(wpa_s->drv_priv, type, NULL,
					     NULL, NULL, NULL, NULL,
					     if_addr, NULL);
	//if (wpa_s->global->p2p) {
	...
	if ( wpa_s->driver->get_802_11_statistics ) {
		unsigned long FailedCount, RetryCount;

		if ( wpa_s->driver->get_802_11_statistics(wpa_s->drv_priv, &FailedCount, &RetryCount) == 0 ) {
			ret = os_snprintf(pos, end - pos, "tx_failed_count=%d\n",
					  FailedCount);
			if (ret < 0 || ret >= end - pos)
				return pos - buf;
			pos += ret;
	
			ret = os_snprintf(pos, end - pos, "tx_retry_count=%d\n",
					  RetryCount);
			if (ret < 0 || ret >= end - pos)
				return pos - buf;
			pos += ret;
				    ...
				    const u8 *ie, size_t ie_len)
{
	...
	if ( data.pairwise_cipher != data.group_cipher ) {
		printf("group_cipher = %x\n", data.group_cipher);

		if (data.group_cipher & WPA_CIPHER_TKIP) {
			ret = os_snprintf(pos, end - pos, "%sGTKIP", first ? "" : "+");
			if (ret < 0 || ret >= end - pos)
				return pos;
			pos += ret;
			first = 0;
		...
		if (data.group_cipher & WPA_CIPHER_WEP104) {
			ret = os_snprintf(pos, end - pos, "%sGWEP104", first ? "" : "+");
			if (ret < 0 || ret >= end - pos)
				return pos;
			pos += ret;
			first = 0;
	...
	const struct wpa_bss *bss, char *buf, size_t buflen)
{
	...
	if (ret < 0 || ret >= end - pos)
		return -1;
	pos += ret;
#if defined (CONFIG_HE_SUPPORT) && !defined (ANDROID)
	/* 
	   HE Special scan format supoort, 
	   the format cannot work under android, result from the extra fields 
	*/
	ret = os_snprintf(pos, end - pos, "%dM\t", bss->max_rate / 2);
	if (ret < 0 || ret >= end - pos)
		return -1;
	pos += ret;

	if ( os_memcmp(bss->name , "11", 2) == 0) 
		ret = os_snprintf(pos, end - pos, "802.%s\t", bss->name);
	else 
	ret = os_snprintf(pos, end - pos, "%s\t", bss->name);
	if (ret < 0 || ret >= end - pos)
		return -1;
	pos += ret;
...
static void scan_results_sort_by_rssi(struct wpa_supplicant *wpa_s)
{
	struct wpa_bss *max_rssi_bss_id;
	struct wpa_bss *bss_id;
	struct dl_list temp_bss_id;
	struct dl_list temp_bss;

	if ( wpa_s->num_bss > 0 ) 
	{
		dl_list_init(&temp_bss_id);
		dl_list_init(&temp_bss);
		while( !dl_list_empty(&wpa_s->bss_id) ) {
			max_rssi_bss_id = NULL;
			bss_id = NULL;

			dl_list_for_each(bss_id, &wpa_s->bss_id, struct wpa_bss, list_id) {
				if ( max_rssi_bss_id == NULL ) {
					max_rssi_bss_id = bss_id;
				} else {
					if ( bss_id->level > max_rssi_bss_id->level) {
						max_rssi_bss_id = bss_id;
					} 
				}
			}
			dl_list_del(&(max_rssi_bss_id->list_id));
			dl_list_add_tail(&temp_bss_id, &(max_rssi_bss_id->list_id));
			dl_list_del(&(max_rssi_bss_id->list));
			dl_list_add_tail(&temp_bss, &(max_rssi_bss_id->list));
		}

		temp_bss_id.next->prev = &wpa_s->bss_id;
		temp_bss_id.prev->next = &wpa_s->bss_id;
		os_memcpy(&wpa_s->bss_id, &temp_bss_id, sizeof(struct dl_list));

		temp_bss.next->prev = &wpa_s->bss;
		temp_bss.prev->next = &wpa_s->bss;
		os_memcpy(&wpa_s->bss, &temp_bss, sizeof(struct dl_list));
	...
	struct wpa_supplicant *wpa_s, char *buf, size_t buflen)
{
	...
	scan_results_sort_by_rssi(wpa_s);
			    ...
			    char *buf, size_t buflen)
{
	...
	if (pos2) {
		pos2 += 11;
		go_intent = atoi(pos2);
		wpa_printf(MSG_DEBUG, "go_intent = %d", go_intent);
...
static int p2p_ctrl_go_ssid(struct wpa_supplicant *wpa_s, char *cmd,
			    char *buf, size_t buflen)
{
	char ssid[32];
	char wpapsk[65];
	...
	if (pos) {
		pos += 5;
		os_memset(ssid, 0, 32);
		os_memcpy(ssid, pos, pos2 - pos);
	}

	if (pos2) {
		pos2 += 8;
		os_memset(wpapsk, 0, sizeof(wpapsk));
		os_memcpy(wpapsk, pos2, os_strlen(pos2));
	}

	wpa_printf(MSG_DEBUG, "%s:: ssid =[%s], wpapsk=[%s]", __func__, ssid, wpapsk);
	wpa_drv_go_ssid(wpa_s, ssid, wpapsk);
...
static int p2p_ctrl_group_add(struct wpa_supplicant *wpa_s, char *cmd)
{
	...
	if ((os_strncmp(cmd, "ssid=", 5) == 0) && os_strstr(cmd, " wpapsk="))
	{        
		return wpas_p2p_group_add_with_ssid_passphrase(wpa_s, 0, freq, ssid, wpapsk);
	}
...
#ifdef CONFIG_WFD
	if (os_strcmp(cmd, "wfdEnable") == 0) {
		return p2p_set_wfd_enable(wpa_s->global->p2p, atoi(param));
	}

	if (os_strcmp(cmd, "devType") == 0) {
		return p2p_set_device_type(wpa_s->global->p2p, atoi(param));
	}
	
	if (os_strcmp(cmd, "sourceCoup") == 0) {
		return p2p_set_source_coupled(wpa_s->global->p2p, atoi(param));
	}
	
	if (os_strcmp(cmd, "sinkCoup") == 0) {
		return p2p_set_sink_coupled(wpa_s->global->p2p, atoi(param));
	}
	
	if (os_strcmp(cmd, "sessionAvail") == 0) {
		return p2p_set_session_available(wpa_s->global->p2p, atoi(param));
	}
	
	if (os_strcmp(cmd, "rtspPort") == 0) {
		return p2p_set_rtsp_port(wpa_s->global->p2p, atoi(param));
	}
	
	if (os_strcmp(cmd, "maxThroughput") == 0) {
		return p2p_set_max_throughput(wpa_s->global->p2p, atoi(param));
	}
#endif /* CONFIG_WFD */
...
int p2p_ctrl_cilent_ageout(struct wpa_supplicant *wpa_s, char *cmd)
{
	u8 addr[ETH_ALEN];
	char *pos;
	unsigned int ageout;
	...
	return wpas_p2p_client_ageout(wpa_s, addr, ageout);
}
					 ...
					 char *buf, size_t *resp_len)
{
	...
	if (os_strcmp(buf, "PING") == 0) {
		...
		if (wpas_wps_cancel(wpa_s))
			reply_len = -1;
	} else if (os_strcmp(buf, "P2P_WPS_CANCEL") == 0) {
		if (wpas_p2p_wps_cancel(wpa_s))
			reply_len = -1;
#ifdef CONFIG_WPS_OOB
		...
		reply_len = p2p_ctrl_connect(wpa_s, buf + 12, reply,
					     reply_size);
	} else if (os_strncmp(buf, "P2P_GO_SSID ", 12) == 0) {
		reply_len = p2p_ctrl_go_ssid(wpa_s, buf + 12, reply,
					     reply_size);
		...
		if (p2p_ctrl_ext_listen(wpa_s, "") < 0)
			reply_len = -1;
	} else if (os_strncmp(buf, "P2P_CLIENT_AGEOUT ", 18) == 0) {
		if (p2p_ctrl_cilent_ageout(wpa_s, buf + 18) < 0)
			reply_len = -1;		
#endif /* CONFIG_P2P */
	} else if (os_strncmp(buf, WPA_CTRL_RSP, os_strlen(WPA_CTRL_RSP)) == 0)
	{
	...
	#ifdef CONFIG_BLOCK_RA0_SCAN_WHEN_P2P_CONNECTION
		int should_scan_wlan = 1;
	
		#ifdef CONFIG_QUERY_P2P_CONNECT_STATE
        		if (wpa_s->driver->get_p2p_connect_state) {
				int connect_state ;
				wpa_s->driver->get_p2p_connect_state(wpa_s->drv_priv, &connect_state);
				if (connect_state > P2P_CONNECT_IDLE)
					should_scan_wlan=0;
        	}
		#endif
		...
		strcpy(wpa_s->conf->scan_ssid, buf + 10 );
		wpa_s->conf->scan_ssid_length = strlen ( buf + 10 );

		printf("%s\n", wpa_s->conf->scan_ssid);
		printf("wpa_s->scan_ssid_length = %d\n", wpa_s->conf->scan_ssid_length);
		...
		if (wpa_supplicant_ctrl_iface_roam(wpa_s, buf + 5))
			reply_len = -1;
	} else if (os_strcmp(buf, "DRIVER_VERSION") == 0) {
		char ver[30];
		
		os_memset(ver, 0, sizeof(ver));
		if (wpa_s->driver->get_driver_version) {
			wpa_s->driver->get_driver_version(wpa_s->drv_priv, ver);
			wpa_msg(wpa_s, MSG_DEBUG, "Driver Version: %s\n", ver);
			/* pass the version back via reply buffer */
			os_memcpy(reply, ver, os_strlen(ver));
			reply[os_strlen(ver)] = '\n';
			reply_len = os_strlen(ver)+1;
		}

#ifdef CONFIG_P2P
	} else if (os_strcmp(buf, "P2P_ASSOC_TAB") == 0) {
		...
		os_memset(&AssocList, 0, sizeof(P2P_STA_ASSOC_LIST));
		if (wpa_s->driver->get_p2p_association_table) {
			wpa_s->driver->get_p2p_association_table(wpa_s->drv_priv, &AssocList);
                    if ( AssocList.maclist_count != 0 ) {
                       for ( i = 0; i < AssocList.maclist_count; i++ ) {
                            wps_dev_type_bin2str(AssocList.device_type[i].dev_type, devtype, WPS_DEV_TYPE_BUFSIZE);
#ifdef CONFIG_WFD
                            sprintf(msg, "P2P_ASSOC_TAB::%02X:%02X:%02X:%02X:%02X:%02X ssid=%s device_type=%s, is_p2p=%d, rssi=%hhd ,is_wfd = %d , rtsp_port = %d", 
#else
                            sprintf(msg, "P2P_ASSOC_TAB::%02X:%02X:%02X:%02X:%02X:%02X ssid=%s device_type=%s, is_p2p=%d, rssi=%hhd", 
#endif /* CONFIG_WFD */
                            AssocList.maclist[i].octet[0], AssocList.maclist[i].octet[1],
                            AssocList.maclist[i].octet[2], AssocList.maclist[i].octet[3],
                            AssocList.maclist[i].octet[4], AssocList.maclist[i].octet[5],
                            AssocList.device_name[i].ssid,devtype , AssocList.is_p2p[i],
                            AssocList.rssi[i]
#ifdef CONFIG_WFD
				,AssocList.is_wfd[i]
				,AssocList.rtsp_port[i]
#endif /* CONFIG_WFD */
);
                            wpa_msg(wpa_s, MSG_INFO, "%s\n", msg);
                           }
                   } else {
                            wpa_msg(wpa_s, MSG_INFO, "P2P_ASSOC_TAB::NOT CONNECTED.\n");
                          }    
				   
				   os_memcpy(reply, &AssocList, (sizeof(P2P_STA_ASSOC_LIST)));
				   reply_len = (sizeof(P2P_STA_ASSOC_LIST));
                }
#else		
		os_memset(&AssocList, 0, sizeof(P2P_STA_ASSOC_LIST));
		if (wpa_s->driver->get_p2p_association_table) {
			wpa_s->driver->get_p2p_association_table(wpa_s->drv_priv, &AssocList);
			for ( i = 0; i < AssocList.maclist_count; i++ ) {
				sprintf(msg, "P2P_ASSOC_TAB::%02X:%02X:%02X:%02X:%02X:%02X ssid=%s  is_p2p=%d", 
					AssocList.maclist[i].octet[0], AssocList.maclist[i].octet[1],
					AssocList.maclist[i].octet[2], AssocList.maclist[i].octet[3],
					AssocList.maclist[i].octet[4], AssocList.maclist[i].octet[5],
					AssocList.device_name[i].ssid, AssocList.is_p2p[i]);
				wpa_msg(wpa_s, MSG_INFO, "%s\n", msg);
			}
		}
#endif /* CONFIG_HE_BD */					
	} else if (os_strcmp(buf, "P2P_OPR_CH") == 0) {
        if (wpa_s->driver->get_p2p_operation_channel) 
        {
    	    char channel;
			wpa_s->driver->get_p2p_operation_channel(wpa_s->drv_priv, &channel);
			reply[0] = channel;
			reply_len = 1;
			wpa_msg(wpa_s, MSG_INFO, "P2P_OPR_CH:0x%x\n", reply[0]);
		}
	}else if(os_strcmp(buf, "P2P_DISCONNECT") == 0){
        if (wpa_s->driver->p2p_disconnect) 
        {
			wpa_printf(MSG_DEBUG, "P2P_DISCONNECT");
			if(wpa_s->driver->p2p_disconnect(wpa_s->drv_priv) < 0)
			{
				reply_len = -1;
			}
		}
	...
	#ifdef CONFIG_QUERY_P2P_CONNECT_STATE
	else if (os_strcmp(buf, "P2P_CON_STATE") == 0) {
        if (wpa_s->driver->get_p2p_connect_state) 
        {
    	    int connect_state;
			wpa_s->driver->get_p2p_connect_state(wpa_s->drv_priv, &connect_state);
			reply[0] = connect_state;
			reply_len = 1;
			wpa_msg(wpa_s, MSG_ERROR, "P2P_CON_STATE:0x%d\n", reply[0]);
		}
	}	
	#endif
	else {
