...
#ifdef CONFIG_WPS_CONFIG_BY_CONF
					, wpa_s->wps
#endif
					);
...
void wpas_start_wps_go(struct wpa_supplicant *wpa_s,
			      struct p2p_go_neg_results *params,
			      int group_formation)
{
	struct wpa_ssid *ssid;
					...
					enum wpa_driver_if_type type)
{
	...
	//	    wpa_s->p2p_group_idx);
	//Carter Added.----->
	os_snprintf(ifname, sizeof(ifname), "p2p%d", wpa_s->p2p_group_idx);
	//Carter Added <-----
...
wpas_p2p_init_group_interface(struct wpa_supplicant *wpa_s, int go)
{
	...
	//group_wpa_s = wpa_supplicant_add_iface(wpa_s->global, &iface);//Carter marked
	group_wpa_s = wpa_s;//Carter added
...
void wpas_go_neg_req_rx(void *ctx, union wpa_event_data *data)
{
	struct wpa_supplicant *wpa_s = ctx;

#if defined (CONFIG_HE_SUPPORT) && !defined (ANDROID)
/* this event is for HE/DTV Linux project before */
	char devtype[WPS_DEV_TYPE_BUFSIZE];
	if (data->p2p_all_info.pri_dev_type == NULL) {
		memset(data->p2p_all_info.pri_dev_type, 0, 8);
        }

	wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR
                " dev_passwd_id=%u" " pri_dev_type=%s name='%s'" " config_methods=0x%04x"
#ifdef CONFIG_WFD
                " rtsp_port=%d"
                " wfd_client=%d"
#endif /* CONFIG_WFD  */
                ,MAC2STR(data->p2p_all_info.dev_addr), data->p2p_all_info.dev_passwd_id,
                wps_dev_type_bin2str(data->p2p_all_info.pri_dev_type, devtype, sizeof(devtype)),
                data->p2p_all_info.dev_name, data->p2p_all_info.config_methods
#ifdef CONFIG_WFD
                ,data->p2p_all_info.rtsp_port
                ,data->p2p_all_info.bWfdClient
#endif /* CONFIG_WFD  */
                );
#else //Native Android event
	/* Android expect GO-NEG-REQ in below format */
        /* P2P-GO-NEG-REQUEST 42:fc:89:a8:96:09 dev_passwd_id=4 */
	wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_REQUEST MACSTR
                " dev_passwd_id=%u",
		MAC2STR(data->p2p_all_info.dev_addr),
		data->p2p_all_info.dev_passwd_id);
#endif
}
		    ...
		    int new_device)
{
	...
	if (info->bWfdClient)
	{
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR
			" p2p_dev_addr=" MACSTR
			" pri_dev_type=%s name='%s' rssi=%hhd config_methods=0x%x dpid=0x%x"
			" dev_capab=0x%x group_capab=0x%x"
			" device_type=0x%x source_coupled=%d sink_coupled=%d"
			" session_avail=%d service_discovery=%d wfd_PC=0x%x wfd_CP=0x%x wfd_time_sync=%d"
			" rtsp_port=%d max_throughput=%d\n\n",
			MAC2STR(addr), MAC2STR(info->p2p_device_addr),
			wps_dev_type_bin2str(info->pri_dev_type, devtype, sizeof(devtype)),
			info->device_name, info->rssi, info->config_methods, info->dpid,
			info->dev_capab, info->group_capab
			, info->wfd_devive_type, info->source_coupled, info->sink_coupled,
			info->session_avail, info->wfd_service_discovery, info->wfd_PC,
			info->wfd_CP, info->wfd_time_sync, info->rtsp_port, info->max_throughput
			);
	}
	else
#endif //for normal Android event
	{
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_DEVICE_FOUND MACSTR
			" p2p_dev_addr=" MACSTR
			" pri_dev_type=%s name='%s' rssi=%hhd config_methods=0x%x dpid=0x%x"
...
#if 0
		printf(P2P_EVENT_DEVICE_FOUND MACSTR
			" p2p_dev_addr=" MACSTR
			" pri_dev_type=%s name='%s' config_methods=0x%x dpid=0x%x"
			" dev_capab=0x%x group_capab=0x%x"
#ifdef CONFIG_WFD
			" device_type=0x%x source_coupled=%d sink_coupled=%d"
			" session_avail=%d service_discovery=%d wfd_PC=0x%x wfd_CP=0x%x wfd_time_sync=%d"
			...
			wps_dev_type_bin2str(info->pri_dev_type, devtype,
						 sizeof(devtype)),
			info->device_name, info->config_methods, info->dpid,
			info->dev_capab, info->group_capab
#ifdef CONFIG_WFD
			, info->wfd_devive_type, info->source_coupled, info->sink_coupled,
			info->session_avail, info->wfd_service_discovery, info->wfd_PC,
...
void wpas_prov_disc_req(void *ctx, const u8 *peer, u16 config_methods,
			...
			,u16 rtsp_port
			,u8 bWfdClient
#endif /* CONFIG_WFD and HE DTV project */
			)
{
	struct wpa_supplicant *wpa_s = ctx;
	char devtype[WPS_DEV_TYPE_BUFSIZE];
	...
	if (config_methods & WPS_CONFIG_DISPLAY) {
		...
		wpa_drv_p2p_display_pin(wpa_s, generated_pin);
...
void wpas_prov_disc_resp(void *ctx, const u8 *peer, u16 config_methods)
{
	...
	if (config_methods == 0x0 )
		wpa_msg(wpa_s, MSG_INFO, "P2P-PROV-DISC-RESP " MACSTR " REJECT",
			MAC2STR(peer));
...
int wpas_p2p_init(struct wpa_global *global, struct wpa_supplicant *wpa_s)
{
	...
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_MGMT) {
		...
		params.intent = wpa_s->conf->p2p_go_intent;
		params.listen_channel = wpa_s->conf->p2p_listen_channel;
		params.op_channel = wpa_s->conf->p2p_oper_channel;
		params.persistent_reconnect = wpa_s->conf->persistent_reconnect;
#ifdef CONFIG_WFD
		params.wfdEnable = wpa_s->conf->wfd_wfdEnable;
		params.devType = wpa_s->conf->wfd_devType;
		params.sessionAvail = wpa_s->conf->wfd_sessionAvail;
		params.sourceCoup = wpa_s->conf->wfd_sourceCoup;
		params.sinkCoup = wpa_s->conf->wfd_sinkCoup;
		params.rtspPort = wpa_s->conf->wfd_rtspPort;
		params.maxThroughput = wpa_s->conf->wfd_maxThroughput;
		params.contentProtect = wpa_s->conf->wfd_contentProtect;
#ifdef CONFIG_P2P
	#ifdef CONFIG_P2P_PERSISTANT_ENABLE
			params.p2pPersistentEn = wpa_s->conf->p2pPersistentEn;
		wpa_printf(MSG_DEBUG, "wpa_s->conf->p2pPersistentEn = %d.\n", wpa_s->conf->p2pPersistentEn);
	#endif
#endif
		
		wpa_printf(MSG_DEBUG, "wpa_s->conf->wfd_wfdEnable = %d.\n", wpa_s->conf->wfd_wfdEnable);
		wpa_printf(MSG_DEBUG, "wpa_s->conf->wfd_devType = %d.\n", wpa_s->conf->wfd_devType);
		wpa_printf(MSG_DEBUG, "wpa_s->conf->wfd_maxThroughput = %d.\n", wpa_s->conf->wfd_maxThroughput);
		wpa_printf(MSG_DEBUG, "wpa_s->conf->wfd_contentProtect = %d.\n", wpa_s->conf->wfd_contentProtect);
#endif /* CONFIG_WFD */

		if (wpa_drv_p2p_set_params(wpa_s, &params) < 0)
		{
			printf("%s:: wpa_drv_p2p_set_params fail !\n", __func__);
			return -1;
		}
		...
		if (global->p2p == NULL)
		{
			...
			if (global->p2p == NULL)
			{
				printf("p2p_data is NULL! FUCK P2P!\n");
				return -1;
			}
		}
		     ...
		     int freq)
{
	...
	if ((pin) && (wps_method == WPS_PIN_KEYPAD))
		os_strlcpy(wpa_s->p2p_pin, pin, sizeof(wpa_s->p2p_pin));
	else if ((pin) && (wps_method == WPS_PIN_DISPLAY))
        {
//             wpa_drv_p2p_display_pin(wpa_s, *(const unsigned int *)pin);
                unsigned int generated_pin;
                generated_pin = strtol(pin, 0, 10);
                wpa_drv_p2p_display_pin(wpa_s, generated_pin);
        }
	else if (wps_method == WPS_PIN_DISPLAY) {//as display case, and no predfined pin bring from UI.
		ret = wps_generate_pin();
		wpa_drv_p2p_display_pin(wpa_s, ret);
...
int wpas_p2p_group_remove(struct wpa_supplicant *wpa_s, const char *ifname)
{
	...
	for (wpa_s = global->ifaces; wpa_s; wpa_s = wpa_s->next) {
        ...
        if (wpa_drv_p2p_group_remove(wpa_s))
                return -1;
...
void wpas_p2p_init_go_params(struct wpa_supplicant *wpa_s,
				    struct p2p_go_neg_results *params,
				    int freq)
{
	u8 bssid[ETH_ALEN];
		       ...
		       int freq)
{
	...
	wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GROUP_STARTED
			...
			"go_dev_addr=" MACSTR "%s",
			wpa_s->ifname,
			wpa_ssid_txt(pparams->ssid, pparams->ssid_len),
			pparams->freq,
			pparams->passphrase ? pparams->passphrase : "",
			MAC2STR(wpa_s->pending_interface_addr),
			pparams->persistent_group ? " [PERSISTENT]" : "");
#endif
#endif

	wpa_drv_start_wps_go(wpa_s, &params);
#if 1
	// Original wpa_supplicant path.
...
/**
 * wpas_p2p_group_add_with_ssid_passphrase - Add a new P2P group with local end as Group Owner
 * @wpa_s: Pointer to wpa_supplicant data from wpa_supplicant_add_iface()
 * @persistent_group: Whether to create a persistent group
 * @freq: Frequency for the group or 0 to indicate no hardcoding
 * @go_ssid: SSID of P2P group
 * @go_passphrase: passphrase of P2P group
 * Returns: 0 on success, -1 on failure
 *
 * This function creates a new P2P group with the local end as the Group Owner,
 * i.e., without using Group Owner Negotiation.
 */
int wpas_p2p_group_add_with_ssid_passphrase(struct wpa_supplicant *wpa_s, int persistent_group,
                int freq, u8 *go_ssid, u8 *go_passphrase)
{
	struct p2p_go_neg_results params, *pparams = &params;
	unsigned int r;
	...
	params.ssid_len = os_strlen(go_ssid);
	os_memcpy(params.ssid, go_ssid, params.ssid_len);	
	os_memcpy(params.passphrase, go_passphrase, os_strlen(go_passphrase));
		       ...
		       const char *config_method)
{
	...
	if (os_strcmp(config_method, "display") == 0)
		config_methods = WPS_CONFIG_DISPLAY;
              ...
              generated_pin = wps_generate_pin();
              wpa_drv_p2p_display_pin(wpa_s, generated_pin);
              wpas_prov_disc_local_display(wpa_s, peer_addr, "", generated_pin);
...
int wpas_p2p_client_ageout(struct wpa_supplicant *wpa_s, const u8 *addr, unsigned int ageout)
{
	if (wpa_s->p2p_group_interface != P2P_GROUP_INTERFACE_GO)
		return -1;
	wpa_printf(MSG_DEBUG, "P2P: Set P2P client %02X:%02X:%02X:%02X:%02X:%02X ageout to %u seconds", 
		*addr, *(addr+1), *(addr+2), *(addr+3), *(addr+4), *(addr+5), ageout);
	return wpa_drv_p2p_client_ageout(wpa_s, addr, ageout);
}
...
void wpas_p2p_update_config(struct wpa_supplicant *wpa_s)
{
	...
	// After updtae params, should we need config driver?
	if (wpa_s->drv_flags & WPA_DRIVER_FLAGS_P2P_MGMT)
	{
		params.intent= wpa_s->conf->p2p_go_intent;
		params.listen_channel = wpa_s->conf->p2p_listen_channel;
		params.dev_name = wpa_s->conf->device_name;
		params.op_channel = wpa_s->conf->p2p_oper_channel;
		os_memcpy(params.pri_dev_type, wpa_s->conf->device_type, WPS_DEV_TYPE_LEN);
#ifdef CONFIG_WFD
		params.wfdEnable = wpa_s->conf->wfd_wfdEnable;
		params.devType = wpa_s->conf->wfd_devType;
		params.sourceCoup= wpa_s->conf->wfd_sourceCoup;
