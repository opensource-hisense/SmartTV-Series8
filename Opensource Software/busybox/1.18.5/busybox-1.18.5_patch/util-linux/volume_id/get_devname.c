...
/* add by polar */
    ...
    #if 1
    /* Skip non sd* device */    
    if ((strncmp(device, "/dev/block/sd", strlen("/dev/block/sd")) != 0) &&
    	(strncmp(device, "/dev/block/vold/sd", strlen("/dev/block/vold/sd")) != 0) &&
    	(strncmp(device, "/dev/block/mmcblk", strlen("/dev/block/mmcblk")) != 0) &&
    	(strncmp(device, "/dev/block/vold/mmcblk", strlen("/dev/block/vold/mmcblk")) != 0)
       )
    {
        return TRUE;
    }
    ...
    /* Skip sdm device */
    if (strncmp(device, "/dev/block/sdm", strlen("/dev/block/sdm")) == 0)
        return TRUE;
#endif
