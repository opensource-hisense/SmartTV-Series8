...
void XMLHttpRequest::createRequest(PassRefPtr<EncodedFormData> httpBody, ExceptionState& exceptionState)
{
    ...
    bool originCheckEnabled = true;
    if (getExecutionContext()->isDocument() && document()->settings() && !document()->settings()->XHROriginCheckEnabled()) {
        originCheckEnabled = false;
    }
    m_sameOriginRequest = !originCheckEnabled || getSecurityOrigin()->canRequestNoSuborigin(m_url);
    ...
    options.crossOriginRequestPolicy = originCheckEnabled ? UseAccessControl : AllowCrossOriginRequests;
