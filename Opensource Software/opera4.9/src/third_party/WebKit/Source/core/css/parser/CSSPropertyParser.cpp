...
static CSSIdSelValue* consumeIdSel(CSSParserTokenRange& range)
{
    if (range.peek().type() != HashToken)
        return nullptr;

    return CSSIdSelValue::create(range.consumeIncludingWhitespace().value().toString());
}

static CSSValue* consumeNavigation(CSSParserTokenRange& range)
{
    if (CSSValue* autoValue = consumeIdent<CSSValueAuto>(range))
        return autoValue;

    CSSIdSelValue* idSel = consumeIdSel(range);
    if (!idSel)
        return nullptr;
    CSSStringValue* target = consumeString(range);
...
static CSSValue* consumeInputFormat(CSSParserTokenRange& range)
{
    ASSERT(RuntimeEnabledFeatures::cssInputFormatEnabled());

    return consumeIdentRange(range, CSSValueAlphaNumeric, CSSValueNumeric);
}
...
CSSValue* CSSPropertyParser::parseSingleValue(CSSPropertyID unresolvedProperty, CSSPropertyID currentShorthand)
{
    ...
    case CSSPropertyInputFormat:
        return consumeInputFormat(m_range);
