...
#ifdef OPERA_TVSDK
    int chrCode = charCode();
    if (type() == EventTypeNames::keydown || type() == EventTypeNames::keyup
        || chrCode == 0) {
        // The condition chrCode == 0 has to be checked, because in some
        // TV specs (notably: HbbTV) not only printable keys generate keypress
        // (e.g. arrows) and if there are no printable results, virtual key code
        // should be returned
