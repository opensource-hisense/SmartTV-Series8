...
{
    if (type == ResourceLoadingFromCache && resource->getStatus() == Resource::Cached && !cachedResource(resource->url()))
    ...
    if (type == ResourceLoadingFromCache && !resource->stillNeedsLoad() && !cachedResource(resource->url())) {
...
ResourceFetcher::RevalidationPolicy ResourceFetcher::determineRevalidationPolicy(Resource::Type type, const FetchRequest& fetchRequest, Resource* existingResource, bool isStaticData) const
{
    ...
    // DVB protocol must not be cached.
    if (request.url().protocolIs("dvb"))
      return Reload;
    ...
    if (type != Resource::Raw) {
        if (!context().isLoadComplete() && cachedResource(existingResource->url()))
            return Use;
...
// Garbage collecting m_documentResources is needed to free the URLs of released resources.
