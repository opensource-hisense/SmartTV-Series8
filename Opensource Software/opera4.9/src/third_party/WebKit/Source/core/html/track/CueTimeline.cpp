...
void CueTimeline::removeCues(TextTrack*, const TextTrackCueVector& cues)
{
  TrackDisplayUpdateScope scope(*this);
  for (const auto& cue : cues)
      removeCue(cue->track(), cue);
}
...
// Fix TTML crash caused by GCC 4.9 bug (?) on Android ARM with -O2, TVSDK-15535
// Force -Os on this function when TVSDK sets global optimization to -O2.
// Note: using !COMPILER(CLANG) rather than COMPILER(GCC) here because clang
// evaluates both to 1 (since it defines __GNUC__) but does not recognize this
// attribute.
#if defined(OPERA_TVSDK) && !COMPILER(CLANG) && defined(__OPTIMIZE__)
