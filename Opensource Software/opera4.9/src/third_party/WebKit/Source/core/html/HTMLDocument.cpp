...
bool HTMLDocument::moveFocusInDirection(WebFocusType type) {
  if (!page() || !page()->settings().spatialNavigationEnabled())
      return false;

  if (page()->focusController().handleCSSFocusNavigation(type))
      return true;
