...
void HTMLParamElement::attributeChanged(const QualifiedName& name, const AtomicString& oldValue, const AtomicString& newValue, AttributeModificationReason reason)
{
    HTMLElement::attributeChanged(name, oldValue, newValue, reason);

    if (name != nameAttr && name != valueAttr)
        return;

    ContainerNode* parent = parentNode();
    if (!isHTMLObjectElement(parent))
        return;

    toHTMLObjectElement(parent)->sendUpdatedParametersToPlugin();
}
