...
void HTMLPlugInElement::resetPluginWidget()
{
...
PluginView* HTMLPlugInElement::getPluginView()
{
    LayoutPart* layoutPart = existingLayoutPart();
    Widget* widget = layoutPart != nullptr ? layoutPart->widget() : nullptr;
    if (!widget)
        widget = m_persistedPluginWidget.get();
...
void HTMLPlugInElement::attach(const AttachContext& context)
{
    ...
    if (PluginView* plugin = getPluginView())
        plugin->didAttachToDocument(context.performingReattach, layoutObject() != nullptr);
    ...
    if (!layoutObject() || useFallbackContent()) {
        ...
        if (m_persistedPluginWidget && !pluginCanExistWithoutLayoutObject()) {
...
bool HTMLPlugInElement::pluginCanExistWithoutLayoutObject() const
{
...
{
    if (!pluginCanExistWithoutLayoutObject())
        return;
...
void HTMLPlugInElement::createPluginWithoutLayoutObject()
{
    ...
    String urlString = m_url;
    String serviceType = m_serviceType;
    Vector<String> paramNames;
    Vector<String> paramValues;

    parametersForPlugin(paramNames, paramValues, urlString, serviceType);

    bool useFallback = false;
    loadPlugin(document().completeURL(urlString), serviceType, paramNames, paramValues, useFallback, false);
}

void HTMLPlugInElement::sendUpdatedParametersToPlugin()
{
    if (PluginView* pluginView = getPluginView()) {
        String url = m_url;
        String serviceType = m_serviceType;
        Vector<String> paramNames;
        Vector<String> paramValues;
        ...
        pluginView->didUpdateParameters(paramNames, paramValues, document().completeURL(url), serviceType);
    }
}
...
void HTMLPlugInElement::detach(const AttachContext& context)
{
    ...
    if (PluginView* plugin = getPluginView())
        plugin->didDetachFromDocument(context.performingReattach, layoutObject() != nullptr);

    // Only try to persist a plugin widget we actually own.
    Widget* plugin = ownedWidget();
    if (plugin && (context.performingReattach || pluginCanExistWithoutLayoutObject())) {
...
LayoutObject* HTMLPlugInElement::createLayoutObject(const ComputedStyle& style)
{
    if (PluginView* plugin = getPluginView()) {
#ifdef OPERA_TVSDK
        // In TV SDK, we need plugins, which are not visual (do not have
        // the compositor layer) not to renderer on page.
        if (!plugin->platformLayer())
            return nullptr;
#endif
    }
...
bool HTMLPlugInElement::hasPluginInstance() const
{
    return m_pluginWrapper || m_persistedPluginWidget;
}
...
SharedPersistent<v8::Object>* HTMLPlugInElement::pluginWrapper()
{
    ...
    // It may happen that we are in the process of creation a plugin
    if (m_isInPluginCreation)
        return nullptr;
...
void HTMLPlugInElement::parametersForPlugin(Vector<String>& paramNames, Vector<String>& paramValues, String& url, String& serviceType)
{
