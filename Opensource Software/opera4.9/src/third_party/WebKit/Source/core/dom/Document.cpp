...
String Document::suggestedMIMEType() const
{
    if (isXMLDocument()) {
        if (isCEHTMLDocument())
            return "application/ce-html+xml";
        if (isXHTMLDocument())
...
Document* Document::cloneDocumentWithoutChildren()
{
    ...
    if (isXMLDocument()) {
        if (isCEHTMLDocument())
            return XMLDocument::createCEHTML(init.withRegistrationContext(registrationContext()));
        if (isXHTMLDocument())
...
void Document::addItemToMap(HashCountedSet<AtomicString>& map, const AtomicString& name)
{
    if (name.isEmpty())
        return;
    map.add(name);
...
void Document::removeItemFromMap(HashCountedSet<AtomicString>& map, const AtomicString& name)
{
    if (name.isEmpty())
        return;
    map.remove(name);
...
void Document::addNamedItem(const AtomicString& name)
{
    addItemToMap(m_namedItemCounts, name);
}

void Document::removeNamedItem(const AtomicString& name)
{
    removeItemFromMap(m_namedItemCounts, name);
}

void Document::addExtraNamedItem(const AtomicString& name)
{
    addItemToMap(m_extraNamedItemCounts, name);
}

void Document::removeExtraNamedItem(const AtomicString& name)
{
