...
void XMLDocumentParser::end()
{
    ...
    if (m_parserPaused) {
        m_waitingForPendingScriptsBeforeEnd = true;
        return;
    }

    endAfterPendingScripts();
}

void XMLDocumentParser::endAfterPendingScripts()
{
    if (m_sawError)
...
XMLDocumentParser::XMLDocumentParser(Document& document, FrameView* frameView)
    ...
    , m_waitingForPendingScriptsBeforeEnd(false)
...
void XMLDocumentParser::resumeParsing()
{
    ...
    // Additonally, if this was a parsing resumed for scripts started
    // after the finish() was called, we need to finalize end() now.
    if (m_waitingForPendingScriptsBeforeEnd && m_pendingCallbacks.isEmpty())
        endAfterPendingScripts();
}
