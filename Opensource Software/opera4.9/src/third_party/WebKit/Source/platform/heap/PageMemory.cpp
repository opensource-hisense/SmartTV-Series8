...
static MemoryCollarCallback s_memoryCollarCallback = nullptr;

void setMemoryCollarFunction(MemoryCollarCallback callback)
{
    s_memoryCollarCallback = callback;
}

void memoryCollarAdjust(int64_t diff)
{
    if (s_memoryCollarCallback)
        s_memoryCollarCallback(diff, false);
}

bool memoryCollarEnabled() {
  return !!s_memoryCollarCallback;
}
...
PageMemory::PageMemory(PageMemoryRegion* reserved, const MemoryRegion& writable)
    : m_reserved(reserved)
    , m_writable(writable)
    , m_startOfUntouchedPages(writable.base())
{
    ...
    // When a memory collar callback is used, the allocator
    // (NormalPageArena) leaves the allocation hotpath to update the
    // count when the current allocation area is expanded into
    // previously untouched system pages (normally every 4KiB).
    // Without a memory collar callback it is a wasted effort, so in
    // that case, the page is setup as if it had no untouched pages
    // from the start. That way, the allocator remains in the hotpath
    // for as long as possible.
    if (!s_memoryCollarCallback)
        m_startOfUntouchedPages += m_writable.size();
}
