...
void PLATFORM_EXPORT setMemoryCollarFunction(MemoryCollarCallback);

void memoryCollarAdjust(int64_t diff);

bool memoryCollarEnabled();
...
public:
    ~PageMemory()
    {
        ...
        // TODO(jb): Rework this so that we don't assume that either
        // the memory has already been decommitted or deleting the
        // page will lead to immediate destruction of the underlying
        // PageMemoryRegion.
        memoryCollarAdjust(m_writable.base() - m_startOfUntouchedPages);
    ...
    void decommit()
    {
        ...
        m_writable.decommit();
        memoryCollarAdjust(m_writable.base() - m_startOfUntouchedPages);
        m_startOfUntouchedPages = m_writable.base();
    }

    Address startOfUntouchedPages() { return m_startOfUntouchedPages; }

    size_t updateStartOfUntouchedPages(Address startOfUntouchedPages)
    {
        if (m_startOfUntouchedPages >= startOfUntouchedPages)
            return 0;

        // Align startOfUntouchedPages to end of system page.
        size_t diff = startOfUntouchedPages - m_startOfUntouchedPages;
        diff += WTF::kSystemPageOffsetMask;
        diff &= WTF::kSystemPageBaseMask;
        m_startOfUntouchedPages += diff;
        memoryCollarAdjust(diff);
        return diff;
    }
...
private:
    ...
    Address m_startOfUntouchedPages;
};
