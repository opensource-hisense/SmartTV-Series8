...
bool V8DebuggerAgentImpl::getFunctionDetailsForInvokableObject(v8::Local<v8::Value> value, std::unique_ptr<FunctionDetails>* details)
{
    if (value.IsEmpty() || value->IsFunction() || !value->IsObject())
        return false;

    v8::Local<v8::Object> functionObject = value.As<v8::Object>();
    if (!functionObject->IsCallable())
        return false;

    std::unique_ptr<protocol::Debugger::Location> location = protocol::Debugger::Location::create()
        .setScriptId(String16::number(0))
        .setLineNumber(0)
        .setColumnNumber(0).build();

    std::unique_ptr<FunctionDetails> functionDetails = FunctionDetails::create()
        .setLocation(std::move(location))
        .setFunctionName(toProtocolStringWithTypeCheck(functionObject->ToString()))
        .setIsGenerator(false).build();
...
void V8DebuggerAgentImpl::getFunctionDetails(ErrorString* errorString, const String16& functionId, std::unique_ptr<FunctionDetails>* details)
{
    ...
    if (!scope.initialize())
        return;
    if (getFunctionDetailsForInvokableObject(scope.object(), details))
        return;
