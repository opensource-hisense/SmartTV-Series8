...
void HTMLSelectElement::menuListDefaultEventHandler(Event* event)
{
    if (event->type() == EventTypeNames::keydown) {
        ...
        // In the delegated menu list rendering case, handling up/down keys and
        // similar on a multiple select element should not happen as we don't
        // actually show all the multiple elements, only a single "status"
        // element that handles the selection process via a popup dialog
        if (multiple() && LayoutTheme::theme().delegatesMenuListRendering())
            return;
