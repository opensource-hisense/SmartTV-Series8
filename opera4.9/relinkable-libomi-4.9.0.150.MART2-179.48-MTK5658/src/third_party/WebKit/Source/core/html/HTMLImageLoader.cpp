...
void HTMLImageLoader::imageNotifyFinished(ImageResource*)
{
    ...
    if (isHTMLImageElement(*element)) {
        if (loadError) {
            // Ligada test case es.tdthibrida_7D7C0220 says we don't want
            // anything to be shown on screen if an image fails to load.
            // And hbbtv pages are CE-HTML
            if (!element->document().isCEHTMLDocument()) {
