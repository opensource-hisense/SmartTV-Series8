...
Document* DOMImplementation::createDocument(const String& type, const DocumentInit& init, bool inViewSourceMode)
{
    ...
    if (type == "application/ce-html+xml" || type == "application/vnd.hbbtv.xhtml+xml")
        return XMLDocument::createCEHTML(init);
    ...
    // Ask HTMLMediaElement about support only for Chromium media MIME types, to prevent creating MediaDocument
    // when media backend supports also text MIME types e.g. text/xml
    if (MIMETypeRegistry::isSupportedMediaMIMEType(type, String(), String())
        && HTMLMediaElement::supportsType(ContentType(type)))
