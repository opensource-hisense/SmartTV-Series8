...
void DOMWindow::close(ExecutionContext* context)
{
    ...
    bool notifyScriptsClosingWindows = settings && settings->notifyScriptsClosingWindows();

    if (notifyScriptsClosingWindows) {
        // If scripts should simply notify the client, notify and return here.
        // The final IPC handler should be responsible for closing the window.
        page->chromeClient().notifyCloseWindow(page->openedByDOM(), frame()->client()->backForwardLength());
        return;
    }
