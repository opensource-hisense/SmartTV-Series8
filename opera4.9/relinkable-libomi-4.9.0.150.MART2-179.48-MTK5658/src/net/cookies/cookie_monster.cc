...
void SortLeastRecentlyAccessed(CookieMonster::CookieItVector::iterator it_begin,
  ...
  CookieMonster::CookieItVector::iterator it_middle = it_begin + num_sort;
  // TVSDK-6258: Only sort 1 more than num_sort if there is at least 1 more
  // entry in the vector. Fix for edge-case when num_sort=(it_end - it_begin)
  if (static_cast<int>(num_sort) < (it_end - it_begin))
    it_middle = it_middle + 1;
  std::partial_sort(it_begin, it_middle, it_end, LRACookieSorter);
}
...
size_t CookieMonster::GetMaxCookies() {
  DCHECK(thread_checker_.CalledOnValidThread());

  return max_cookies_;
}

bool CookieMonster::SetMaxCookies(size_t max_cookies) {
  // Be sure we do not mess with the runnnig garbage collection.
  DCHECK(thread_checker_.CalledOnValidThread());

  if (max_cookies == max_cookies_)
    return true;


  // The GC will be triggerred by next cookie update.
  max_cookies_ = max_cookies;
  // For an explanation of this refer to k{Max|Purge}Cookies documentation
  purge_cookies_ = max_cookies_/(kMaxCookies/kPurgeCookies);
...
size_t CookieMonster::GarbageCollect(const Time& current,
  ...
  if (cookies_.size() > max_cookies_ && earliest_access_time_ < safe_date) {
