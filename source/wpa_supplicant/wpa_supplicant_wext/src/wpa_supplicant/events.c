			    ...
			    struct wpa_ssid **selected_ssid)
{
	...
	for (prio = 0; prio < wpa_s->conf->num_prio; prio++) {
		...
		for (ssid = wpa_s->conf->pssid[prio] ; ssid; ssid = ssid->pnext) {
			if ( ssid != wpa_s->conf->pssid[prio] ) {
				if ( (ssid->proto == WPA_PROTO_RSN ) && (ssid->proto > wpa_s->conf->pssid[prio]->proto) ) {
					struct wpa_ssid *temp;
					struct wpa_ssid *priv;

					for (priv = wpa_s->conf->pssid[prio] ; priv; priv = priv->pnext)
						if ( priv->pnext == ssid )
							break;

					temp = ssid->pnext;
					ssid->pnext = wpa_s->conf->pssid[prio]->pnext;
					wpa_s->conf->pssid[prio]->pnext = temp;

					priv->pnext = wpa_s->conf->pssid[prio];

					temp = wpa_s->conf->pssid[prio];
					
					wpa_s->conf->pssid[prio] = ssid;
					ssid = temp;
				}
			}
            /* protection from infinite loop */
			  ...
			  union wpa_event_data *data)
{
	...
	case EVENT_P2P_DEV_FOUND: {
		...
		if (data->p2p_all_info.dev_addr)
			os_memcpy(peer_info.p2p_device_addr,
				  data->p2p_all_info.dev_addr, ETH_ALEN);
		if (data->p2p_all_info.pri_dev_type)
			os_memcpy(peer_info.pri_dev_type,
				  data->p2p_all_info.pri_dev_type,
				  sizeof(peer_info.pri_dev_type));
		if (data->p2p_all_info.dev_name)
			os_strlcpy(peer_info.device_name,
				   data->p2p_all_info.dev_name,
				   sizeof(peer_info.device_name));
		peer_info.config_methods = data->p2p_all_info.config_methods;
		peer_info.dev_capab = data->p2p_all_info.dev_capab;
		peer_info.group_capab = data->p2p_all_info.group_capab;
		peer_info.rssi = data->p2p_all_info.rssi;
#if defined (CONFIG_WFD)
		peer_info.bWfdClient = data->p2p_all_info.bWfdClient;
		peer_info.wfd_devive_type = data->p2p_all_info.wfd_devive_type;
		peer_info.source_coupled = data->p2p_all_info.source_coupled;
		peer_info.sink_coupled = data->p2p_all_info.sink_coupled;
		peer_info.session_avail = data->p2p_all_info.session_avail;
		peer_info.wfd_service_discovery = data->p2p_all_info.wfd_service_discovery;
		peer_info.wfd_PC = data->p2p_all_info.wfd_PC;
		peer_info.wfd_CP = data->p2p_all_info.wfd_CP;
		peer_info.wfd_time_sync = data->p2p_all_info.wfd_time_sync;
		peer_info.rtsp_port = data->p2p_all_info.rtsp_port;
		peer_info.max_throughput = data->p2p_all_info.max_throughput;
		if (data->p2p_all_info.assoc_addr)
			os_memcpy(peer_info.assoc_addr,
				  data->p2p_all_info.assoc_addr, ETH_ALEN);
#endif /* Special event for HE/DTV project with WFD function */
		 ...
		 */
		wpas_dev_found(wpa_s, data->p2p_all_info.addr, &peer_info, 1);
		{
			struct p2p_device *dev;
			struct p2p_data *p2p = wpa_s->global->p2p;
			dev = p2p_driver_create_device(wpa_s->global->p2p, &peer_info.p2p_device_addr);

			if (dev == NULL)
				printf("%s:: create dev fail.\n", __FUNCTION__);
			else
			{
				dev->flags |= 0x1000; // P2P_DEV_GROUP_CLIENT_ONLY
				os_memcpy(dev->info.device_name, peer_info.device_name, sizeof(peer_info.device_name));
				dev->info.dev_capab = peer_info.dev_capab;
				dev->info.group_capab = peer_info.group_capab;
				dev->info.config_methods = peer_info.config_methods;
				os_memcpy(dev->info.pri_dev_type, peer_info.pri_dev_type, 8);
				dev->info.rssi = peer_info.rssi;
				os_memcpy(dev->interface_addr, data->p2p_all_info.peer, ETH_ALEN);
#if defined (CONFIG_WFD)
				dev->info.bWfdClient = peer_info.bWfdClient;
				dev->info.wfd_devive_type = peer_info.wfd_devive_type;
				dev->info.source_coupled = peer_info.source_coupled;
				dev->info.sink_coupled = peer_info.sink_coupled;
				dev->info.session_avail = peer_info.session_avail;
				dev->info.wfd_service_discovery = peer_info.wfd_service_discovery;
				dev->info.wfd_PC = peer_info.wfd_PC;
				dev->info.wfd_CP = peer_info.wfd_CP;
				dev->info.wfd_time_sync = peer_info.wfd_time_sync;
				dev->info.rtsp_port = peer_info.rtsp_port;
				dev->info.max_throughput = peer_info.max_throughput;
#endif /* Special event for HE/DTV project with WFD function */
			}
		}
		break;
...
*/
	...
	case EVENT_P2P_PROV_DISC_REQUEST:
		wpas_prov_disc_req(wpa_s, data->p2p_all_info.peer,
				   data->p2p_all_info.config_methods,
				   data->p2p_all_info.dev_addr,
				   data->p2p_all_info.pri_dev_type,
				   data->p2p_all_info.dev_name,
				   data->p2p_all_info.supp_config_methods,
				   data->p2p_all_info.dev_capab,
				   data->p2p_all_info.group_capab
#ifdef CONFIG_WFD && defined (CONFIG_HE_SUPPORT)
					,data->p2p_all_info.rtsp_port
					,data->p2p_all_info.bWfdClient
#endif /* CONFIG_WFD and HE DTV projects*/
					);
		break;
	case EVENT_P2P_PROV_DISC_RESPONSE:
		...
		break;
	case EVENT_P2P_INVITE_REQUEST:
	{
		char devtype[WPS_DEV_TYPE_BUFSIZE];
		
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_INVITATION_RECEIVED MACSTR
			" p2p_dev_addr=" MACSTR
			" pri_dev_type=%s name='%s' ",
			MAC2STR(data->p2p_prov_disc_req.peer),
			MAC2STR(data->p2p_prov_disc_req.dev_addr),
			wps_dev_type_bin2str(data->p2p_prov_disc_req.pri_dev_type, devtype,sizeof(devtype)),
	...
	case EVENT_P2P_WPS_COMPLETED:
		printf("%s::  P2P WPS Success!\n", __FUNCTION__);		
		wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_SUCCESS);
		break;

	case EVENT_P2P_GO_NEG_COMPLETED:
		printf("%s::  P2P Negotiate Success!\n", __FUNCTION__);
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GO_NEG_SUCCESS);
		break;

	case EVENT_P2P_CONNECTED:
	{
		int freq = 0;

		printf("%s::  P2P Connected!\n", __FUNCTION__);
		freq = (2407 + (data->p2p_all_info.P2PChannel * 5));
		/*
			for Android, we shall follow below format to report GROUP_STARTED
		...
		*/
		...
		//for HE support, this event is from Alan's Special driver for DTV LINUX platform, 
		//in normal driver and normal Android, the below event will never need.
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GROUP_STARTED
			"rule %s bssid=" MACSTR " ssid=%s peer=" MACSTR
			" is_peer5G=%d"
			" opchannel=%d"
			" is _SCCChannel=%d"
#ifdef CONFIG_WFD
			" rtsp_port=%d  device_type=%d"
#endif /* CONFIG_WFD */
			,
			(data->p2p_all_info.Rule == 1)? "GO" : "CLIENT",
			MAC2STR(data->p2p_all_info.Bssid),
			data->p2p_all_info.Ssid,
			MAC2STR(data->p2p_all_info.peer),
			data->p2p_all_info.Is_peer5G,
			data->p2p_all_info.P2PChannel,
			data->p2p_all_info.Is_SCCChannel
#ifdef CONFIG_WFD
			,data->p2p_all_info.rtsp_port,data->p2p_all_info.wfd_devive_type
#endif /* CONFIG_WFD */
			);
#else //for normal Android event
		char psk[65] = {0};
		os_memcpy(psk, data->p2p_all_info.WpaPsk, 64);
		psk[64] = '\0';
		wpa_msg(wpa_s, MSG_INFO, P2P_EVENT_GROUP_STARTED
			"%s %s ssid=\"%s\" freq=%d passphrase=\"%s\" go_dev_addr=" MACSTR "%s"
#if defined (CONFIG_WFD) && defined (CONFIG_HE_SUPPORT)
                        " rtsp_port=%d device_type=%d"
#endif /* Special event for HE/DTV project with WFD function */
			,
			wpa_s->ifname, (data->p2p_all_info.Rule == 1)? "GO" : "CLIENT",
			data->p2p_all_info.Ssid,
			freq,
			psk,
			(data->p2p_all_info.Rule == 1)? MAC2STR(data->p2p_all_info.Bssid) : MAC2STR(data->p2p_all_info.peer), ""
#if defined (CONFIG_WFD) && defined (CONFIG_HE_SUPPORT)
                        ,data->p2p_all_info.rtsp_port, data->p2p_all_info.wfd_devive_type
#endif /* Special event for HE/DTV project with WFD function */
			);
#endif
		/* If my rule is GO, get current ssid for invitation */
		if (data->p2p_all_info.Rule == 1)
		{
			struct p2p_go_neg_results params;

			wpas_p2p_init_go_params(wpa_s, &params, freq);
			strcpy(params.ssid, data->p2p_all_info.Ssid);
			params.ssid_len = strlen(data->p2p_all_info.Ssid);
			os_memcpy(params.passphrase, data->p2p_all_info.WpaPsk, sizeof(params.passphrase));
			wpa_s = wpas_p2p_get_group_iface(wpa_s, 0, 1);
			if (wpa_s == NULL)
				break;
			wpas_start_wps_go(wpa_s, &params, 1);
			if (wpa_s->current_ssid != NULL)
				printf("wpa_s->current_ssid->ssid: %s, wpa_s->current_ssid->ssid_len: %d\n", wpa_s->current_ssid->ssid, wpa_s->current_ssid->ssid_len);
		}
	}
		break;
	case EVENT_P2P_DISCONNECTED:
		/*
			Android format
		...
		*/
		...
		printf("%s::  P2P Disconnected!\n", __FUNCTION__);
#if defined (CONFIG_HE_SUPPORT) && !defined (ANDROID) //for HE Linux platform event
		wpas_p2p_disconnect(wpa_s);
#else //for normal Android event
		wpa_msg(wpa_s->parent, MSG_INFO, P2P_EVENT_GROUP_REMOVED "%s %s reason=REQUESTED",
		wpa_s->ifname, (data->p2p_group_report.Rule == 1)? "GO" : "CLIENT");
#endif

		break;
	case EVENT_P2P_LEGACY_CONNECTED:
		printf("%s::  P2P Legacy Connected %02X:%02X:%02X:%02X:%02X:%02X !\n", __FUNCTION__,
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		wpa_msg(wpa_s, MSG_INFO, "P2P-LEGACY-CONNECT %02X:%02X:%02X:%02X:%02X:%02X",
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		break;
	case EVENT_P2P_LEGACY_DISCONNECTED:
		printf("%s::  P2P Legacy Disconnected %02X:%02X:%02X:%02X:%02X:%02X !\n", __FUNCTION__,
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		wpa_msg(wpa_s, MSG_INFO, "P2P-LEGACY-DISCONNECT %02X:%02X:%02X:%02X:%02X:%02X",
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		break;
	case EVENT_P2P_AP_STA_CONNECTED:
		wpa_printf(MSG_DEBUG, "%s::  P2P AP STA CONNECTED %02X:%02X:%02X:%02X:%02X:%02X !\n", __FUNCTION__,
			...
			/* p2p_peer return interface address not correct */
			{
				struct p2p_device *dev = p2p_get_device(wpa_s->global->p2p,data->p2p_ap_sta_connected.dev_addr);

				if(dev != NULL)
					os_memcpy(dev->interface_addr,data->p2p_ap_sta_connected.addr,ETH_ALEN);
			}
#if defined (CONFIG_HE_SUPPORT) && !defined (ANDROID) //for HE Linux platform event
		wpa_msg(wpa_s, MSG_INFO, AP_STA_CONNECTED
			MACSTR " dev_addr=" MACSTR " is_go=%d update_table=%d"
#ifdef CONFIG_WFD
			" rtsp_port=%d"
#endif /* CONFIG_WFD */
			, MAC2STR(data->p2p_ap_sta_connected.addr), MAC2STR(data->p2p_ap_sta_connected.dev_addr),
			data->p2p_ap_sta_connected.bIsGo, data->p2p_ap_sta_connected.bInsPerTable
#ifdef CONFIG_WFD
			,data->p2p_ap_sta_connected.rtsp_port
#endif /* CONFIG_WFD */
			);
#else //for normal Android event
		if (data->p2p_ap_sta_connected.dev_addr)
			wpa_msg(wpa_s, MSG_INFO, AP_STA_CONNECTED
				MACSTR " dev_addr=" MACSTR,
				MAC2STR(data->p2p_ap_sta_connected.addr), MAC2STR(data->p2p_ap_sta_connected.dev_addr));
		else
			wpa_msg(wpa_s, MSG_INFO, AP_STA_CONNECTED
				MACSTR, MAC2STR(data->p2p_ap_sta_connected.addr));
#endif /* CONFIG_HE_SUPPORT & !ANDROID */
		break;
	case EVENT_P2P_AP_STA_DISCONNECTED:
		wpa_printf(MSG_DEBUG, "%s::  P2P AP STA DISCONNECTED %02X:%02X:%02X:%02X:%02X:%02X !\n", __FUNCTION__,
			data->p2p_ap_sta_connected.addr[0], data->p2p_ap_sta_connected.addr[1],
			data->p2p_ap_sta_connected.addr[2], data->p2p_ap_sta_connected.addr[3],
			data->p2p_ap_sta_connected.addr[4], data->p2p_ap_sta_connected.addr[5]);
		...
		if (data->p2p_ap_sta_connected.dev_addr)
			wpa_msg(wpa_s, MSG_INFO, AP_STA_DISCONNECTED
				MACSTR " dev_addr=" MACSTR,
				MAC2STR(data->p2p_ap_sta_connected.addr), MAC2STR(data->p2p_ap_sta_connected.dev_addr));
		else
			wpa_msg(wpa_s, MSG_INFO, AP_STA_DISCONNECTED
				MACSTR, MAC2STR(data->p2p_ap_sta_connected.addr));
		break;
	case EVENT_P2P_DEVICE_TABLE_ITEM_DELETE:
		wpa_msg(wpa_s, MSG_INFO, "P2P-DEVICE-TABLE-ITEM-DELEE " MACSTR, MAC2STR(data->p2p_dev_found.dev_addr));
		break;
#endif /* CONFIG_P2P */
	...
	case EVENT_IBSS_PEER_LOST:
		...
		break;
#if defined (CONFIG_HE_SUPPORT) && !defined (ANDROID) //for HE Linux platform event
	case EVENT_P2P_GO_NEGO_FAIL_INTENT:
		wpa_printf(MSG_DEBUG, "%s::  P2P GO NEGO FAIL INTENT %02X:%02X:%02X:%02X:%02X:%02X!\n", __FUNCTION__,
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		wpa_msg(wpa_s, MSG_INFO, "P2P-GO-NEGO-FAIL-INTENT " MACSTR,
			MAC2STR(data->new_sta.addr));
		break;
	case EVENT_P2P_GO_NEGO_FAIL_RECV_RESP:
		wpa_printf(MSG_DEBUG, "%s::  P2P GO NEGO FAIL RECV RESP %02X:%02X:%02X:%02X:%02X:%02X status_code=%d!\n", __FUNCTION__,
			data->fail_status.addr[0], data->fail_status.addr[1], 
			data->fail_status.addr[2], data->fail_status.addr[3], 
			data->fail_status.addr[4], data->fail_status.addr[5], data->fail_status.rv);
		wpa_msg(wpa_s, MSG_INFO, "P2P-GO-NEGO-FAIL-RECV-RESP " MACSTR " status_code=%d",
			MAC2STR(data->fail_status.addr), data->fail_status.rv);
		break;
	case EVENT_P2P_GO_NEGO_FAIL_RECV_CONFIRM:
		wpa_printf(MSG_DEBUG, "%s::  P2P GO NEGO FAIL RECV CONFIRM %02X:%02X:%02X:%02X:%02X:%02X status_code=%d!\n", __FUNCTION__,
			data->fail_status.addr[0], data->fail_status.addr[1], 
			data->fail_status.addr[2], data->fail_status.addr[3], 
			data->fail_status.addr[4], data->fail_status.addr[5], data->fail_status.rv);
		wpa_msg(wpa_s, MSG_INFO, "P2P-GO-NEGO-FAIL-RECV-CONFIRM " MACSTR " status_code=%d",
			MAC2STR(data->fail_status.addr), data->fail_status.rv);
		break;
	case EVENT_P2P_WPS_FAIL:
		wpa_printf(MSG_DEBUG, "%s::  P2P WPS FAIL %02X:%02X:%02X:%02X:%02X:%02X status_code=%d!\n", __FUNCTION__,
			data->fail_status.addr[0], data->fail_status.addr[1], 
			data->fail_status.addr[2], data->fail_status.addr[3], 
			data->fail_status.addr[4], data->fail_status.addr[5], data->fail_status.rv);
		wpa_msg(wpa_s, MSG_INFO, "P2P-WPS-FAIL " MACSTR " status_code=%d",
			MAC2STR(data->fail_status.addr), data->fail_status.rv);
		break;
	case EVENT_P2P_WPS_TIMEOUT:
		wpa_printf(MSG_DEBUG, "%s::  P2P WPS TIMEOUT %02X:%02X:%02X:%02X:%02X:%02X!\n", __FUNCTION__,
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		wpa_msg(wpa_s, MSG_INFO, "P2P-WPS-TIMEOUT " MACSTR,
			MAC2STR(data->new_sta.addr));
		break;
	case EVENT_P2P_WPS_2MINS_TIMEOUT:
		wpa_printf(MSG_DEBUG, "%s::  P2P WPS 2MINS TIMEOUT!\n", __FUNCTION__);		
		wpa_msg(wpa_s, MSG_INFO, "P2P-WPS-2MINS-TIMEOUT");
		break;
	case EVENT_P2P_WPS_PBC_SESSION_OVERLAP:
		wpa_printf(MSG_DEBUG, "%s::  P2P WPS PBC SESSION OVERLAP!\n", __FUNCTION__);		
		wpa_msg(wpa_s, MSG_INFO, "P2P-WPS-PBC-SESSION-OVERLAP");
		break;
	case EVENT_P2P_WPS_SEND_M2D:
		wpa_printf(MSG_DEBUG, "%s::  P2P WPS SEND M2D!\n", __FUNCTION__);		
		wpa_msg(wpa_s, MSG_INFO, "P2P-WPS-SEND-M2D");
		break;
	case EVENT_P2P_WPS_RECEIVE_NACK:
		wpa_printf(MSG_DEBUG, "%s::  P2P WPS RECEIVE NACK!\n", __FUNCTION__);		
		wpa_msg(wpa_s, MSG_INFO, "P2P-WPS-RECEIVE-NACK");
		break;
	case EVENT_P2P_STOP_CONNECT:
		wpa_printf(MSG_DEBUG, "%s::  P2P STOP CONNECT %02X:%02X:%02X:%02X:%02X:%02X!\n", __FUNCTION__,
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		wpa_msg(wpa_s, MSG_INFO, "P2P-STOP-CONNECT " MACSTR,
			MAC2STR(data->new_sta.addr));
		break;
	case EVENT_P2P_DEAUTH_AP:
		wpa_printf(MSG_DEBUG, "%s::  P2P DEAUTH AP %02X:%02X:%02X:%02X:%02X:%02X!\n", __FUNCTION__,
			data->new_sta.addr[0], data->new_sta.addr[1], 
			data->new_sta.addr[2], data->new_sta.addr[3], 
			data->new_sta.addr[4], data->new_sta.addr[5]);
		wpa_msg(wpa_s, MSG_INFO, "P2P_DEAUTH_AP " MACSTR,
			MAC2STR(data->new_sta.addr));
		break;
#endif /* CONFIG_HE_SUPPORT & !ANDROID */
	default:
