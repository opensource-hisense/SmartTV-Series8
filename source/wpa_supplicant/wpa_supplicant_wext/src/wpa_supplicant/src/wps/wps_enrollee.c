...
static int wps_process_CredsSecurityOrder(struct wps_credential *wps_cred)
{
    u16 auth_type = 0;
    u16 encr_type = 0;
    u16 BitMask;
    ...
    {
      if (wps_cred->auth_type & BitMask)
      {
        auth_type = wps_cred->auth_type & BitMask;
        break;
      }
    }
    for(BitMask = WPS_ENCR_AES; BitMask != 0; BitMask = BitMask>>1)
    {
      if (wps_cred->encr_type & BitMask)
      {
        encr_type = wps_cred->encr_type & BitMask;
        break;
      }
    }
    switch (encr_type)
    {
        ...
        case WPS_ENCR_TKIP:
        	if(auth_type == WPS_AUTH_WPAPSK)
              i4Ret = WPS_ORDER_WPA_TKIP;
        	else if(auth_type == WPS_AUTH_WPA2PSK)
          	 i4Ret = WPS_ORDER_WPA_2_TKIP;
        	else 
          	 i4Ret = WPS_ORDER_NONE;
        	break;	
        case WPS_ENCR_AES:
        	if(auth_type == WPS_AUTH_WPAPSK)
              i4Ret = WPS_ORDER_WPA_AES;
        	else if(auth_type == WPS_AUTH_WPA2PSK)
          	 i4Ret = WPS_ORDER_WPA_2_AES;
        	else 
          	 i4Ret = WPS_ORDER_NONE;
        	break;	
        default:
...
//    WPS_INFO("WPS: Security Order %d\n", i4Ret);
    wpa_printf(MSG_DEBUG, "WPS: Security Order %d\n", i4Ret);
    return i4Ret;
...
//notify credential and set to connection list in "wps_process_cred_e" , 
//We select one max security credential here
#ifdef CONFIG_WPS_MAX_SECURITY
	if (num_cred > 1) {
		wpa_printf(MSG_DEBUG, "WPS: credential number = %d", num_cred);
		struct wps_parse_attr attr;
		struct wpabuf msg;
		int i4SecurityOrder[num_cred];
		int i4Select = -1;
		int i4SecurityMax = 0;

		for (i = 0; i < num_cred; i++) 
		{
			wpa_printf(MSG_DEBUG, "WPS: Pre_parse Multiple Credential %d", num_cred);
			os_memset(&wps->cred, 0, sizeof(wps->cred));
			wpabuf_set(&msg, cred[i], cred_len[i]);
			if (wps_parse_msg(&msg, &attr) < 0 ||wps_process_cred(&attr, &wps->cred))
			{
				wpa_printf(MSG_DEBUG, "WPS: Pre_parse fail, move to next one");
				continue;
			}
			else
			{
				i4SecurityOrder[i] = wps_process_CredsSecurityOrder(&wps->cred);
					if(i4SecurityOrder[i] >= i4SecurityMax)
					{
						i4Select = i;
						i4SecurityMax =  i4SecurityOrder[i];
					}
				}
			}
      		 ...
      		 if(i4Select == -1)   
	       {
      		...
      		wpa_printf(MSG_DEBUG, "WPS: select Credential No.%d in %d creds, Security %d\n", i4Select, num_cred, i4SecurityMax);

		int res;
		res = wps_process_cred_e(wps, cred[i4Select], cred_len[i4Select], wps2);
		if (res == 0)
			ok++;
		else if (res == -2)
