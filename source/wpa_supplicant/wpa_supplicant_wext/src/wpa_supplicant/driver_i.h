				      ...
				      int persistent_group)
{
	...
	return wpa_s->driver->p2p_connect(wpa_s->drv_priv, peer_addr,
					  ...
					  persistent_group, wpa_s->p2p_pin);
}

static inline int wpa_drv_start_wps_go(struct wpa_supplicant *wpa_s,
                                        struct p2p_go_neg_results *params)
{
        if ( ! wpa_s->driver->set_p2p_force_go)
                return -1;
        return wpa_s->driver->set_p2p_force_go(wpa_s->drv_priv, 1, params);
}

static inline int wpa_drv_go_ssid(struct wpa_supplicant *wpa_s,
                                        const char *ssid, const char *wpapsk)
{
        if ( !wpa_s->driver->set_p2p_go_ssid)
                return -1;
        return wpa_s->driver->set_p2p_go_ssid(wpa_s->drv_priv, ssid, wpapsk);
}

static inline int wpa_drv_start_pin(struct wpa_supplicant *wpa_s,
                                        const char *pin)
{
        if ( !wpa_s->driver->p2p_start_pin)
                return -1;
        return wpa_s->driver->p2p_start_pin(wpa_s->drv_priv, pin);
}

static inline int wpa_drv_start_pbc(struct wpa_supplicant *wpa_s,
                                        u8 *enable)
{
        if ( !wpa_s->driver->p2p_start_pbc)
                return -1;
        return wpa_s->driver->p2p_start_pbc(wpa_s->drv_priv, enable);
}

static inline int wpa_drv_p2p_group_remove(struct wpa_supplicant *wpa_s)
{
        if ( ! wpa_s->driver->set_p2p_force_go)
                return -1;
        return wpa_s->driver->set_p2p_force_go(wpa_s->drv_priv, 0, NULL);
}

static inline int wpa_drv_p2p_disconnect(struct wpa_supplicant *wpa_s)
{
        if ( ! wpa_s->driver->p2p_disconnect)
                return -1;
        return wpa_s->driver->p2p_disconnect(wpa_s->drv_priv);
}
...
static inline int wpa_drv_p2p_display_pin(struct wpa_supplicant *wpa_s,
					 const unsigned int generated_pin)
{
	if (!wpa_s->driver->p2p_display_pin)
		return -1;
	return wpa_s->driver->p2p_display_pin(wpa_s->drv_priv, generated_pin);
}
...
static inline int wpa_drv_p2p_client_ageout(struct wpa_supplicant *wpa_s,
				     const u8 *addr, unsigned int ageout)
{
	if (!wpa_s->driver->set_p2p_client_ageout)
		return -1;
	return wpa_s->driver->set_p2p_client_ageout(wpa_s->drv_priv, addr, ageout);
}
