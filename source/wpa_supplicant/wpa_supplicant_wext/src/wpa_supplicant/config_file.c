...
static void wpa_config_write_global(FILE *f, struct wpa_config *config)
{
...
#ifdef CONFIG_WFD
	if (config->wfd_wfdEnable)
		fprintf(f, "wfd_wfdEnable=%u\n",
			config->wfd_wfdEnable);
	if (config->wfd_devType)
		fprintf(f, "wfd_devType=%u\n",
			config->wfd_devType);
	if (config->wfd_sourceCoup)
		fprintf(f, "wfd_sourceCoup=%u\n",
			config->wfd_sourceCoup);
	if (config->wfd_sinkCoup)
		fprintf(f, "wfd_sinkCoup=%u\n",
			config->wfd_sinkCoup);
	if (config->wfd_sessionAvail)
		fprintf(f, "wfd_sessionAvail=%u\n",
			config->wfd_sessionAvail);
	if (config->wfd_rtspPort)
		fprintf(f, "wfd_rtspPort=%u\n",
			config->wfd_rtspPort);
	if (config->wfd_maxThroughput)
		fprintf(f, "wfd_maxThroughput=%u\n",
			config->wfd_maxThroughput);
	if (config->wfd_contentProtect)
		fprintf(f, "wfd_contentProtect=%u\n",
			config->wfd_contentProtect);
#endif /* CONFIG_WFD */
	...
	if (config->p2pPersistentEn)
		fprintf(f, "p2pPersistentEn=%u\n", config->p2pPersistentEn);
	#endif
