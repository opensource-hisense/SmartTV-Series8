...
static u8 selected_bssid[ETH_ALEN];
extern int b_scan_only;
extern int b_connect;
#endif
					    ...
					    int only_first_ssid)
{
	...
	for (ssid = group; ssid; ssid = only_first_ssid ? NULL : ssid->pnext) {
		...
		/*Mac address compare*/
		if(b_connect){
			if (os_memcmp(bss->bssid, selected_bssid, ETH_ALEN) != 0) {
				wpa_dbg(wpa_s, MSG_DEBUG, "   skip - Mac address "
					"not match");
				continue;
			}
		}
...
#ifdef CONFIG_BSS_HANDLE
		if(b_scan_only && !b_connect){
			os_memcpy(selected_bssid, selected->bssid, ETH_ALEN*(sizeof(u8)));
			wpa_printf(MSG_DEBUG, DEBUGFMT"   selected BSS " MACSTR
				" ssid='%s' ",
				DEBUGARGS,
				MAC2STR(selected->bssid),
				wpa_ssid_txt(selected->ssid, selected->ssid_len));

			//notify event to framework
			wpa_msg(wpa_s, MSG_INFO, WPA_EVENT_MAC_SELECTED "wlan0_mac_addr=" 
				MACSTR,
				MAC2STR(selected_bssid));
			
			//wpa_supplicant_rsn_preauth_scan_results(wpa_s);
			return 1;
		}else if(b_scan_only && b_connect){
			wpa_printf(MSG_DEBUG, DEBUGFMT, DEBUGARGS);
			...
			if (stype == WLAN_FC_STYPE_PROBE_REQ &&
				...
				size_t ie_len = data->rx_mgmt.frame_len -
					(mgmt->u.probe_req.variable -
					 data->rx_mgmt.frame);
#ifdef CONFIG_MTK_HE_SUPPORT
                wpa_dbg(wpa_s, MSG_EXCESSIVE, "%s:Receive ProbeReq from " MACSTR ,__FUNCTION__, MAC2STR(src));
#endif
