				       ...
				       const u8 *ie, size_t ie_len)
{
	...
	if (dev) {
        ...
        /* Fix sometimes we send GO Neg Req on wrong channel 
         * because peer has changed its Listen channel.
         * We should update its P2P info and WFD info upon every Probe Req. 
         */
        if (msg.listen_channel) {
            os_memcpy(dev->country, msg.listen_channel, 3);
			...
			enum p2p_send_action_result result)
{
	...
	p2p->pending_action_type = P2P_PENDING_ACTION_NONE;
#endif
	p2p->pending_action_state = P2P_NO_PENDING_ACTION;
	switch (state) {
...
#ifdef CONFIG_SHARED_P2P_MAC_ADDR
	p2p->pending_action_type = P2P_PENDING_ACTION_VENDOR_SPECIFIC;
#endif
	p2p->pending_action_state = P2P_NO_PENDING_ACTION;
...
int p2p_set_go_intent(struct p2p_data *p2p, u8 go_intent)
{
	p2p->go_intent = go_intent;
	return 0;
}
...
int p2p_set_oper_freq(struct p2p_data *p2p, int* p2p_oper_freq, int* p2p_oper_reg_class, int* p2p_oper_channel, u8* acs_channel, int channel_len)
{	
	p2p_dbg(p2p, "P2P: Get p2p oper freq:reg_class %u channel %u p2p_op_freq %u",
		p2p->cfg->op_reg_class, p2p->cfg->op_channel, p2p->p2p_oper_freq);
	...
	//TODO, set opchannel
	if (*p2p_oper_freq == 2 || *p2p_oper_freq == 5)
	{
		op_channel = p2p_get_best_channel(p2p, acs_channel, channel_len, *p2p_oper_freq, &op_reg_class);
		wpa_printf(MSG_DEBUG, "p2p_set_oper_freq get ACS channel is %d", op_channel);
	}
	if (op_channel == 0 && (*p2p_oper_freq == 2 || *p2p_oper_freq == 5))
	{
		wpa_printf(MSG_ERROR, "Can't get right operation channel from ACS");
		return -1;
	}
	
	if(2 == *p2p_oper_freq || 5 == *p2p_oper_freq)
	{
		//ACS channel,TODO		
		p2p->cfg->op_reg_class = op_reg_class;
		p2p->cfg->op_channel = op_channel;
	...
	{
		if(0 == p2p_freq_to_channel(*p2p_oper_freq,
			&op_reg_class, &op_channel))	
		{
			p2p->cfg->op_reg_class = op_reg_class;
			p2p->cfg->op_channel = op_channel;
			p2p->cfg->cfg_op_channel = 1;
			p2p->p2p_oper_freq = *p2p_oper_freq;
	...
	if(0 == ret_err)
	{
		*p2p_oper_freq = p2p->p2p_oper_freq;
		*p2p_oper_reg_class = p2p->cfg->op_reg_class;
		*p2p_oper_channel = p2p->cfg->op_channel;
		
		p2p_dbg(p2p, "P2P: Set oper freq success: reg_class %u channel %u p2p_op_freq %u",
			p2p->cfg->op_reg_class, p2p->cfg->op_channel, p2p->p2p_oper_freq);
	}
...
int p2p_get_best_channel(struct p2p_data *p2p, u8* acs_channel, int channel_len, int freq, u8* op_reg_class)
{
	int count = 0;
	u8 channel;
	int p2p_oper_freq;
	...
	if (freq == 2)
	{
		for (count = 0; count < channel_len; ++count)
		{
			channel = acs_channel[count];
			
			if (channel == 14)
				p2p_oper_freq = 2484;
			else
				p2p_oper_freq = 2407 + channel * 5;	

			if(!p2p_freq_to_channel(p2p_oper_freq, op_reg_class, &channel)
					&& p2p_channels_includes(&p2p->cfg->channels, *op_reg_class, channel))
			{
				p2p->p2p_oper_freq = p2p_oper_freq;
				return channel;
			}
...
int p2p_get_listen_freq(struct p2p_data *p2p)
{
	return  p2p_channel_to_freq(p2p->cfg->reg_class,
					   p2p->cfg->channel);
}
#endif
...
#ifdef CONFIG_SUPPORT_GET_P2P_INFOR
int p2p_get_own_interface_address(struct p2p_data *p2p, u8* if_addr)
{
	if (p2p == NULL || is_zero_ether_addr(p2p->intended_addr))
		return -1;
	os_memcpy(if_addr, p2p->intended_addr, ETH_ALEN);
	return 0;
}

int p2p_get_own_device_name(struct p2p_data *p2p, char* device_name)
{
	if (p2p != NULL && p2p->cfg->dev_name != NULL)
		os_memcpy(device_name, p2p->cfg->dev_name, os_strlen(p2p->cfg->dev_name));
	return 0;
}

int p2p_get_own_pri_dev_type(struct p2p_data *p2p, u8* pri_dev_type)
{
	os_memcpy(pri_dev_type, p2p->cfg->pri_dev_type, 8);
	return 0;
}

u16 p2p_get_own_config_methods(struct p2p_data *p2p)
{
	return p2p->cfg->config_methods;
}

u8 p2p_get_own_dev_capab(struct p2p_data *p2p)
{
	 return p2p->dev_capab;
}

u8 p2p_get_own_group_capab(struct p2p_data *p2p)
{
	return p2p->group_capab;
}
