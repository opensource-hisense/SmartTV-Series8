				...
				const u8 *data, size_t len, int rx_freq)
{
	...
	if(p2p->pending_action_state != P2P_NO_PENDING_ACTION)
	{
		wpa_printf(MSG_ERROR, "Drop P2P Invitation Request while p2p->pending_action_state(%d) != P2P_NO_PENDING_CATION", p2p->pending_action_state);
		return;
	}
#endif
	...
	if (op_freq) {
...
#ifndef CONFIG_SUPPORT_SET_OP_FREQ	
...
#else
				/*Only if we are GC, we need to refer peer op channel*/
				if(go == 0)
				{
					/* Use peer preference if specified and compatible */
					if (msg.operating_channel) {
        ...
        /* only if we are GO can go here*/
        if ((go != 0) && (!p2p->cfg->cfg_op_channel || !p2p_channels_includes(&intersection, p2p->op_reg_class,
					   p2p->op_channel)))
...
void p2p_invitation_resp_cb(struct p2p_data *p2p, int success)
{
	...
	wpa_s = (struct wpa_supplicant* )p2p->cfg->cb_ctx;
	void wpas_p2p_remove_pending_group_interface(struct wpa_supplicant *wpa_s);
#endif
	if (!success)
	{
#ifdef CONFIG_HANDLE_P2P_INVITATION_RSP_TX_STATUS
		wpa_printf(MSG_ERROR, "Back to p2p listen/p2p search when no action frame ack!!!!");
