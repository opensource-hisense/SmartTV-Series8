			       ...
			       struct p2p_message *msg)
{
	...
	case WFD_SUBELEM_DEVICE_INFO:
		if (len != 6) {
			wpa_printf(MSG_DEBUG, "P2P: Too short Wfd Device info "
				   "attribute (length %d)", len);
			return -1;
		}
		wpa_printf(MSG_DEBUG, "P2P: * Wfd Device info %02x%02x%02x%02x%02x%02x "
			   ,pos[0], pos[1],pos[2],pos[3],pos[4],pos[5]);
		break;
	case WFD_SUBELEM_ASSOCIATED_BSSID:
		if (len != ETH_ALEN) {
			wpa_printf(MSG_DEBUG, "P2P: Too short Wfd Associated bssid "
				   "attribute (length %d)", len);
			return -1;
		}
		msg->wfd_assoc_bssid = pos;
		wpa_printf(MSG_DEBUG, "P2P : * Wfd Associated bssid " MACSTR,
			   MAC2STR(msg->wfd_assoc_bssid));
		break;
	default:
		wpa_printf(MSG_DEBUG, "P2P: Skipped unknown attribute %d "
			   "(length %d)", id, len);
...
int p2p_parse_wfd_ie(const struct wpabuf *buf, struct p2p_message *msg)
{
	...
	wpa_printf(MSG_EXCESSIVE, "P2P: Parsing WFD IE");
...
#ifdef CONFIG_SUPPORT_GET_WFD_ASSOCBSSID_IN_CONN_MSG
		if (msg->wfd_subelems &&
			p2p_parse_wfd_ie(msg->wfd_subelems, msg)) {
